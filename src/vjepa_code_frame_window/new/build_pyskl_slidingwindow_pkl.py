...
================================================================
Same window setup as V-JEPA v2:
  - 30 frames (2s at 15fps) per window
  - 15 frame stride (1s)
  - Majority-vote label per window
  - Includes "None" class (background)

Outputs pyskl-format pkl files for training/val/test.

Usage:
    python build_pyskl_slidingwindow_pkl.py --task locomotion
    python build_pyskl_slidingwindow_pkl.py --task rmm
"""
...
def main():
    ...
    parser.add_argument("--task", required=True, choices=["locomotion", "rmm", "both"])
    ...
    for task in tasks:
        ...
        print(f"  Label map ({num_classes} classes): {label_map}")
        ...
        # PoseC3D (best for locomotion)
        generate_posec3d_config(task, pkl_path, num_classes, config_dir)

        # CTR-GCN with jm (best for rmm)
        generate_ctrgcn_config(task, pkl_path, num_classes, "jm", config_dir)

    print(f"\n{'='*60}")
    print("DONE! Next steps:")
    print("  1. Run: python build_pyskl_slidingwindow_pkl.py --task both")
    print("  2. Train with the SLURM script below")
    print(f"{'='*60}")
