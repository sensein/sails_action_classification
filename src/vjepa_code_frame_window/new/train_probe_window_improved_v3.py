"""
Improved Sliding-Window Attentive Probe — v3
==============================================
Builds on v2 (which beat all baselines) by adding ONE thing:
  - Single transformer encoder layer before cross-attention
    (lets frames interact with each other within the window)

Everything else stays identical to v2:
  - Sinusoidal positional encoding
  - 2 query tokens
  - Gentle class-weighted CE (strength=0.5)
  - Confidence-weighted frame voting at inference
  - Dropout 0.1

Usage:
    python train_probe_window_improved_v3.py --task locomotion --seed 42
    python train_probe_window_improved_v3.py --task rmm --seed 42
"""
...
OUTPUT_BASE   = "/orcd/data/satra/002/projects/SAILS/vjepa_features/models_output_seeds/vjepa/window_improved/v3/"
...
NUM_QUERIES   = 2
NUM_HEADS     = 8
NUM_ENC_LAYERS = 1          # <-- the ONE new thing vs v2
...
# 4. IMPROVED PROBE v3
# ============================================================
class ImprovedAttentiveProbeV3(nn.Module):
    """
    v2 + single transformer encoder layer.
    The encoder lets frames attend to each other before pooling,
    so the model can detect transitions and temporal patterns
    within the window.
    """
    def __init__(self, embed_dim, num_classes, num_heads=NUM_HEADS,
        ...
        # Single transformer encoder layer (frame-to-frame interaction)
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=embed_dim,
            nhead=num_heads,
            dim_feedforward=embed_dim * 2,   # 2x instead of typical 4x — lightweight
            dropout=dropout,
            activation="gelu",
            batch_first=True,
        ...
        # Multi-query cross-attention pooling (same as v2)
    ...
    def forward(self, x):
        ...
        # Frame-to-frame self-attention (1 layer)
        x = self.encoder(x)
...
def main():
    ...
    # ── Model ──────────────────────────────────────────────
    probe = ImprovedAttentiveProbeV3(
        embed_dim=EMBED_DIM,
