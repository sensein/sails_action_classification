...
class Evidence:
    ...
    p_rigidity: Optional[float] = None  # 0=rigid/picture, 1=natural motion
...
def compute_keypoint_rigidity_score(
    ...
    Real people have natural articulation - relative distances between joints vary
    as they move. Pictures/static objects maintain perfectly rigid relative geometry.

    This is camera-motion invariant because we measure RELATIVE distances between
    keypoints, not absolute positions.

    Args:
        keypoints_list: List of keypoint arrays for each frame
        min_confidence: Minimum confidence for keypoint visibility
        min_frames: Minimum frames needed for analysis

    Returns:
        Rigidity score: 0.0 = rigid/static (picture), 1.0 = natural motion (real person)
    """
    if not NUMPY_AVAILABLE or not keypoints_list or len(keypoints_list) < min_frames:
        return None

    try:
        # Measure multiple inter-keypoint distances over time
        # Real people: these distances vary with pose changes
        # Pictures: these distances are perfectly constant (rigid transformation)

        distance_series = {
            'shoulder_width': [],      # L-shoulder to R-shoulder
            'hip_width': [],           # L-hip to R-hip
            'torso_height': [],        # Shoulder-mid to hip-mid
            'left_upper_arm': [],      # L-shoulder to L-elbow
            'right_upper_arm': [],     # R-shoulder to R-elbow
            'left_forearm': [],        # L-elbow to L-wrist
            'right_forearm': [],       # R-elbow to R-wrist
            'left_thigh': [],          # L-hip to L-knee
            'right_thigh': [],         # R-hip to R-knee
            'left_shank': [],          # L-knee to L-ankle
            'right_shank': [],         # R-knee to R-ankle
        }
        ...
        for kp in keypoints_list:
            ...
            # Helper to get keypoint if confident
            def get_kp(idx):
                if idx < len(kp_array) and kp_array[idx][2] > min_confidence:
                    return kp_array[idx][:2]
                return None
            ...
            if l_shoulder is not None and r_shoulder is not None:
                distance_series['shoulder_width'].append(np.linalg.norm(r_shoulder - l_shoulder))

            if l_hip is not None and r_hip is not None:
                distance_series['hip_width'].append(np.linalg.norm(r_hip - l_hip))

                if l_shoulder is not None and r_shoulder is not None:
                    shoulder_mid = (l_shoulder + r_shoulder) / 2
                    hip_mid = (l_hip + r_hip) / 2
                    distance_series['torso_height'].append(np.linalg.norm(shoulder_mid - hip_mid))

            if l_shoulder is not None and l_elbow is not None:
                distance_series['left_upper_arm'].append(np.linalg.norm(l_shoulder - l_elbow))

            if r_shoulder is not None and r_elbow is not None:
                distance_series['right_upper_arm'].append(np.linalg.norm(r_shoulder - r_elbow))

            if l_elbow is not None and l_wrist is not None:
                distance_series['left_forearm'].append(np.linalg.norm(l_elbow - l_wrist))

            if r_elbow is not None and r_wrist is not None:
                distance_series['right_forearm'].append(np.linalg.norm(r_elbow - r_wrist))

            if l_hip is not None and l_knee is not None:
                distance_series['left_thigh'].append(np.linalg.norm(l_hip - l_knee))

            if r_hip is not None and r_knee is not None:
                distance_series['right_thigh'].append(np.linalg.norm(r_hip - r_knee))

            if l_knee is not None and l_ankle is not None:
                distance_series['left_shank'].append(np.linalg.norm(l_knee - l_ankle))

            if r_knee is not None and r_ankle is not None:
                distance_series['right_shank'].append(np.linalg.norm(r_knee - r_ankle))

        # Compute coefficient of variation (CV) for each distance series
        # CV = std / mean (normalized variability)
        # Pictures: CV ≈ 0 (distances perfectly constant despite camera motion)
        # Real people: CV > 0.1 (distances vary with natural articulation)

        cvs = []
        for name, distances in distance_series.items():
            ...
            if mean_dist > 1e-6:  # Avoid division by zero
        ...
        # Average CV across all measured distances
        avg_cv = float(np.mean(cvs))

        # Map CV to rigidity score
        # Pictures: CV < 0.05 (very rigid) → score = 0.0
        # Real people: CV > 0.15 (natural articulation) → score = 1.0
        # Linear interpolation between

        if avg_cv < 0.05:
            return 0.0  # Rigid (likely picture)
        elif avg_cv > 0.15:
            return 1.0  # Natural motion (real person)
        else:
...
class SigLipModel:
    ...
    def load_siglip_model(self):
        ...
        try:
            self.siglip_model = SiglipForImageClassification.from_pretrained("prithivMLmods/Age-Classification-SigLIP2")
            self.siglip_processor = AutoImageProcessor.from_pretrained("prithivMLmods/Age-Classification-SigLIP2")
        except Exception:
...
class SingleChildIdentifier:
    ...
    def __init__(self, cfg: ChildIdentificationConfig, annotations: AnnotationInfo, siglip_model: Optional[SigLipModel] = None):
        self.cfg = cfg
    ...
    def _compute_rigidity_prob(self, tl: Tracklet, flags: List[str]) -> Optional[float]:
        """Compute rigidity score to detect static pictures vs real people.

        Uses keypoint articulation analysis - measures coefficient of variation
        in inter-keypoint distances over time. This is camera-motion invariant.

        Returns:
            Score: 0.0 = rigid/static (picture), 1.0 = natural motion (real person)
        """
        if not NUMPY_AVAILABLE:
            flags.append("numpy_not_available")
            return None
    ...
    def _compute_evidence(self, tl: Tracklet) -> Evidence:
        ...
        Stage 1.5 (Rigidity): Keypoint articulation analysis to reject static pictures
        ...
        - Compute rigidity score to detect static pictures.
        ...
        # Rigidity evidence (Stage 1.5)
        p_rigidity: Optional[float] = None
    ...
    def _score_node(self, tl: Tracklet) -> NodeScore:
        ...
        # Hard filter: reject tracks with very low rigidity (static pictures)
        if self.cfg.enable_rigidity_detection and ev.p_rigidity is not None:
            if ev.p_rigidity < self.cfg.min_rigidity_score:
