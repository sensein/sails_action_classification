...
NEGATIVE_PROMPT = "cat, dog, animal, pet, toys"

# Minimum confidence score to accept a detection (raise if too many FPs)
MIN_CONFIDENCE = 0.25

# Bidirectional overlap threshold: reject ONLY if BOTH the toddler mask
# covers this much of the animal mask AND vice versa (= same physical object).
# 0.5 means 50% mutual overlap — conservative, avoids rejecting kids near pets.
ANIMAL_OVERLAP_THRESHOLD = 0.5
...
def is_animal_false_positive(toddler_mask: np.ndarray,
    ...
    """
    Check if a toddler detection is actually an animal by checking if the
    toddler mask and an animal mask cover the SAME object (high mutual overlap).

    This uses BIDIRECTIONAL overlap: the toddler mask must overlap heavily
    with the animal mask AND the animal mask must overlap heavily with the
    toddler mask. This means they are segmenting the same physical object.

    When a child is playing WITH a dog, the toddler mask covers the child
    and the dog mask covers the dog — different regions, low mutual overlap,
    so the toddler detection is KEPT.

    When a cat is falsely detected as "toddler", both the toddler mask and
    the animal mask cover the same cat — high mutual overlap, so it is REJECTED.
    ...
    Args:
        ...
        overlap_threshold: reject if BOTH directions exceed this ratio
    ...
    for animal_mask in animal_masks:
        # How much of the "toddler" is covered by this animal?
        toddler_covered = compute_mask_overlap_ratio(toddler_mask, animal_mask)
        # How much of this animal is covered by the "toddler"?
        animal_covered  = compute_mask_overlap_ratio(animal_mask, toddler_mask)

        # Both must be high → same physical object detected by both prompts
        if toddler_covered >= overlap_threshold and animal_covered >= overlap_threshold:
            return True
...
def pick_top1_human_only(obj_ids, scores, masks_np, boxes_xywh,
    ...
    for idx in sorted_indices:
        ...
        # Only reject if the toddler mask and animal mask cover the SAME object
        # (bidirectional high overlap). A child near/touching an animal will NOT
        # be rejected because their masks cover different regions.
        if masks_np is not None and idx < len(masks_np) and animal_masks is not None:
            ...
            if is_animal_false_positive(candidate_mask, animal_masks, overlap_threshold):
                print(f"      ⚠ Rejected detection ID={obj_ids[idx]} "
                      f"(score={score:.3f}) — same object detected as animal")
                continue
