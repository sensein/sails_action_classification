...
OUTPUT_BASE = "/orcd/data/satra/002/projects/SAILS/vjepa_features/models_output_seeds/fusion/sw_fusion_v2/"
...
# ============================================================
# 2. MAP WINDOWS TO PER-FRAME SCORES
# ============================================================
...
def build_unified_labels(vjepa_label_map, pyskl_int_to_label):
    ...
    vjepa_str_to_unified = {
        lbl: all_labels.index(lbl)
        for lbl in vjepa_labels if lbl in all_labels
    }
    pyskl_int_to_unified = {
        idx: all_labels.index(lbl)
        for idx, lbl in pyskl_int_to_label.items() if lbl in all_labels
    }
    return all_labels, vjepa_str_to_unified, pyskl_int_to_unified


# ============================================================
# 5. V-JEPA SOFT SCORES
# ============================================================
...
def fuse_one_video(vjepa_df, pyskl_windows, all_labels,
    ...
    vjepa_scores              = vjepa_to_frame_scores(
        vjepa_df, all_labels, vjepa_str_to_unified
    )
    pyskl_raw, pyskl_counts   = windows_to_frame_scores(
...
def run_all_seeds(task, alphas):
    seed_results = [run_one_seed(task, seed, alphas) for seed in SEEDS]
    ...
    for alpha in alphas:
        fa = [r[alpha]["frame_acc"]  for r in seed_results]
        aa = [r[alpha]["action_acc"] for r in seed_results]
...
def main():
    ...
    for task in tasks:
        if args.seed is not None:
            run_one_seed(task, args.seed, args.alphas)
        else:
            run_all_seeds(task, args.alphas)
