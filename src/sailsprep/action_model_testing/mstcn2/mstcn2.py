...
# ============================================================
# MS-TCN++ MODEL (corrected: dual dilated PG + separate R layers)
# ============================================================
...
class PredictionGeneration(nn.Module):
    ...
    def __init__(self, num_layers, num_f_maps, dim, num_classes):
        ...
        self.conv_dilated_1 = nn.ModuleList()  # decreasing dilation
        self.conv_dilated_2 = nn.ModuleList()  # increasing dilation
...
class MSTCNLoss(nn.Module):
    def __init__(self, num_classes, smoothing_weight=0.15, clamp=16.0):
        super().__init__()
...
# DATA LOADING
# ============================================================
def load_feature_file(path, expected_dim):
...
def load_label_sequence(label_path, label_col, label_map, num_frames):
    try:
...
def gather_all_labels(df_csv, label_col):
    all_labels = {BACKGROUND_LABEL}
...
class FullVideoDataset(Dataset):
    def __init__(self, samples, label_col, label_map, feature_col, feature_dim):
    ...
    def __getitem__(self, idx):
        ...
        T    = feat.shape[1]
        labels = load_label_sequence(label_path, self.label_col,
                                     self.label_map, T)
...
def collate_fn(batch):
    ...
    feats_pad  = torch.zeros(len(feats), D,     T_max)
...
# ============================================================
class MSTCNTrainer:
    def __init__(self, num_classes, feature_dim, output_dir, label_map, seed):
        self.num_classes = num_classes
        self.feature_dim = feature_dim
    ...
    def train(self, train_samples, val_samples, label_col, feature_col, device):
        ...
        for epoch in range(start_epoch, NUM_EPOCHS + 1):
            ...
            for feats, labels, masks, _ in train_dl:
                ...
                valid       = (labels != -100)
                n_correct  += ((preds == labels) & valid).sum().item()
            ...
            with torch.no_grad():
                for feats, labels, masks, _ in val_dl:
                    ...
                    preds     = outputs[-1].argmax(dim=1)
                    valid     = (labels != -100)
                    nv_correct += ((preds == labels) & valid).sum().item()
                    nv_total   += valid.sum().item()
        ...
        # ---- Training done: remove resume checkpoint so re-runs skip straight to predict ----
        if os.path.exists(resume_ckpt):
    ...
    def predict(self, test_samples, label_col, feature_col, ckpt_path, device):
    ...
    def evaluate(self, all_true, all_pred, tag="test"):
        ...
        per_class_dict = {lab: float(f) for lab, f in zip(labels_sorted, per_class_f1)}
    ...
    def _save_segment_summary(self, rows, seed_dir):
...
# ============================================================
def aggregate_seeds(output_dir, seeds, tag="test"):
...
def main():
    ...
    if args.action == "train":
        ...
        # Auto-predict + evaluate after training
        if test_samples:
