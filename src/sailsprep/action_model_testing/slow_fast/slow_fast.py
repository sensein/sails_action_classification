...
class BBoxCropVideoDataset(Dataset):
    def __init__(self, samples, label_map, num_frames=NUM_FRAMES,
                 crop_size=CROP_SIZE, alpha=ALPHA, training=False):
    ...
    def _pack_pathway(self, frames):
        fast     = frames
        slow_idx = torch.linspace(0, frames.shape[1] - 1,
...
class SlowFastFineTune(pl.LightningModule):
    def __init__(self, num_classes, freeze_backbone=FREEZE_BACKBONE,
                 class_weights=None):
        super().__init__()
...
def main():
    ...
    if actual_classes != num_classes:
        print(f"  WARNING: config NUM_CLASSES={num_classes} but found "
