...
ANN_FPS = 15.0
MIN_FRAMES = 15  # < 15 annotation-frames 
...
def find_action_runs(ann, label_col):
    ...
    while i < n:
        ...
        if lab in ("N/A", ""):
            ...
            continue
        j = i
        while j + 1 < n and labels[j + 1].strip() == lab and frames[j + 1] == frames[j] + 1:
            j += 1
...
def build_samples(split_csv, label_col):
    ...
    split_buckets = defaultdict(list)  # "train" / "val" / "test" -> [sample, ...]
    ...
    for _, row in df_csv.iterrows():
        ...
        for sf, ef, lab in runs:
            clips = chunk_run(sf, ef)  # apply clipping rules
            for cs, ce in clips:
...
class H5BBoxDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        # Save label mapping + test split info
        os.makedirs(self.output_dir, exist_ok=True)
        with open(os.path.join(self.output_dir, "label_mapping.json"), "w") as f:
            json.dump(self.label_map, f, indent=2)
        pd.DataFrame(test_s).to_csv(os.path.join(self.output_dir, "test_split.csv"), index=False)

        # Inverse-frequency class weights from TRAIN split only
...
class SlowFastFineTune(pl.LightningModule):
    def __init__(self, num_classes, freeze_backbone=FREEZE_BACKBONE, class_weights=None):
        super().__init__()
    ...
    def validation_step(self, batch, _):
        inputs, labels = batch
        preds = self.model(inputs)
        loss = F.cross_entropy(preds, labels)  # unweighted for honest monitoring
        acc = (preds.argmax(1) == labels).float().mean()
...
def run_inference(model, test_samples, label_map, device, output_dir):
    ...
    if len(valid):
        ...
        cm = confusion_matrix(valid["true_label"], valid["pred_label"], labels=labels)
...
def main():
    ...
    # Sanity-check: num_classes in config vs actual classes found
