...
=====================================================
Run with: python ablation_study.py --version v2
...
def build_dataset_from_folders(clips_dir):
    ...
    classes = sorted([d for d in os.listdir(clips_dir)
                      if os.path.isdir(os.path.join(clips_dir, d)) and d != "logs"])
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if MODEL_NAME == "slow_r50":
                dummy = torch.zeros(3, NUM_FRAMES, CROP_SIZE, CROP_SIZE)
            else:
...
class ClipDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if USE_CLASS_WEIGHTS:
            ...
            print(f"\nClass weights: {dict(zip(classes, weights.round(2)))}")
        ...
        if MODEL_NAME == "slow_r50":
            ...
            train_post = [PackPathway()]
            eval_post = [PackPathway()]
...
def run_inference(model, test_df, label_map, device):
    ...
    if len(valid) > 0:
        ...
        print(f"{'=' * 60}")
        print(f"Total: {len(results_df)} | Processed: {len(valid)} | Errors: {len(results_df) - len(valid)}")
        print(f"Accuracy: {accuracy:.4f} ({int(valid['correct'].sum())}/{len(valid)})")

        report = classification_report(valid["true_label"], valid["predicted_label"], zero_division=0)
