...
Usage:
    python window_classifier_qwen.py --task loco --approach a --csv split.csv
    python window_classifier_qwen.py --task loco --approach a --csv split.csv \
...
# ──────────────────────────────────────────────────────────────
FRAME_PREAMBLE = (
    "This is a single frame from a 2-second video clip of a young "
    "child. Based on the child's body position, limb placement, and "
    "posture in this frame, determine what movement is happening."
)
...
LOCO_BINARY_DESC = (
    ...
    "with no movement in any direction."
)
...
class QwenClassifier:
    def __init__(
        ...
        *,
        ...
        self.model.eval()
        print(f"Qwen loaded on {next(self.model.parameters()).device}.")
    ...
    def _finegrained_prompt(self) -> str:
        ...
        return (
            "This is a single frame from a 2-second video clip of a "
            "young child who IS performing an active movement. Based on "
            "the child's body position and posture, identify which "
            "specific movement.\n\n"
            f"{definitions}\n\n"
            f"Possible movements: {classes_str}\n\n"
            "Respond with exactly one line:\n"
    ...
    # ------------------------------------------------------------------
    # Parsers — FIXED: no aggressive fallback keywords
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Approach A — multi-class, per-frame majority vote
    # ------------------------------------------------------------------
    def classify_multiclass(
        ...
        for fi, img in enumerate(images):
            try:
                raw = self._call(img, prompt)
                print(
                    f"  [DEBUG] clip={clip_index} frame={fi} raw={raw!r}"
                )
        ...
        return best, preds, counter[best] / len(preds)

    # ------------------------------------------------------------------
    # Approach B — 2-stage, per-frame majority vote
    # ------------------------------------------------------------------
    def classify_2stage(
        self, video_path: str, start_sec: float, end_sec: float,
        clip_index: int,
        ...
        bin_prompt = self._binary_prompt()
        votes: list[bool] = []
        for fi, img in enumerate(images):
            try:
                raw = self._call(img, bin_prompt)
                print(
                    f"  [DEBUG] clip={clip_index} frame={fi} "
                    f"binary_raw={raw!r}"
                )
        ...
        pos_count = sum(votes)
        confidence = pos_count / len(votes)
        if pos_count <= len(votes) / 2:
            return (
                self.no_label, confidence, self.no_label,
                [self.no_label] * len(votes), self.no_label,
            )

        fine_prompt = self._finegrained_prompt()
        fine_preds: list[str] = []
        for fi, img in enumerate(images):
            try:
                raw = self._call(img, fine_prompt)
                print(
                    f"  [DEBUG] clip={clip_index} frame={fi} "
                    f"fine_raw={raw!r}"
                )
                pred = self._parse_finegrained(raw)
                if pred:
        ...
        if not fine_preds:
            ...
            return self.binary_pos, confidence, fallback, [fallback], fallback

        counter = Counter(fine_preds)
        best = counter.most_common(1)[0][0]
        return self.binary_pos, confidence, best, fine_preds, best

    # ------------------------------------------------------------------
    # Approach C — binary, per-frame majority vote
    # ------------------------------------------------------------------
    def classify_binary(
        self, video_path: str, start_sec: float, end_sec: float,
        clip_index: int,
        ...
        confidence = pos_count / len(votes)
        label = (
            self.binary_pos if pos_count > len(votes) / 2
            else self.no_label
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Unified 2-sec window classifier (Qwen2.5-VL).",
    )
    ...
    print(f"\n{'=' * 60}")
    print(f"Qwen2.5-VL — task={args.task}  approach={args.approach}")
    print(f"{'=' * 60}")
