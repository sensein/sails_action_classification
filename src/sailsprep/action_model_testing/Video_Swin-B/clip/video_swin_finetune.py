"""End-to-end Video Swin-B (K400 pretrained) fine-tuning for Loco and RMM.
...
# ── Action-run extraction and clipping ───────────────────────────────────────
...
def build_samples(
    ...
    Args:
        split_csv: Path to the master split CSV (``latest_split_csv.csv``).
...
def collate_fn(
    ...
    Args:
        batch: List of ``(video_tensor, class_index)`` tuples from the dataset.
...
class H5BBoxDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:  # noqa: ARG002
        ...
        if n_train == 0:
            raise RuntimeError(
                "No training clips built — check paths and label column."
            )
...
class VideoSwinFineTune(pl.LightningModule):
    ...
    def configure_optimizers(self) -> dict:
        """Set up AdamW optimizer with ReduceLROnPlateau scheduler.
