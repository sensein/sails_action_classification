"""Video Swin-B binary classifier: N/A vs non-N/A on 2-sec sliding windows.
...
def build_samples(
    ...
    for _, row in split_df.iterrows():
        ...
        while start + window_ann_frames <= total_ann_frames + stride_ann_frames:
            ...
            binary_label = get_window_binary_label(frame_to_label, start, end)
            by_split[sp].append(
                {
                    "video_path": vp,
                    "h5_path": hp,
...
def collate_fn(
    ...
    """Stack video tensors and labels into batched tensors."""
    videos, labels = zip(*batch)
    return torch.stack(list(videos)), torch.tensor(
...
class H5BBoxDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader:  # type: ignore[type-arg]
        ds = BBoxCropVideoDataset(
            self.train_samples, self.label_map or {}, training=True
        )
        return DataLoader(
            ds,
    ...
    def val_dataloader(self) -> DataLoader:  # type: ignore[type-arg]
        ds = BBoxCropVideoDataset(
            self.val_samples, self.label_map or {}, training=False
        )
        return DataLoader(
            ds,
            batch_size=BATCH_SIZE,
...
class VideoSwinClassifier(nn.Module):
    """Classification wrapper around Video Swin-B backbone.

    Args:
        backbone: The Video Swin-B backbone module.
        feat_dim: Dimensionality of the backbone's output features.
        num_classes: Number of target classes (2 for binary).
    """
...
def run_inference(
    ...
    for i in range(len(ds)):
        ...
        try:
            ...
            if (i + 1) % 50 == 0:
                print(
                    f"  [{i + 1}/{len(ds)}]"
                    f" {os.path.basename(str(s['video_path']))}"
    ...
    if len(valid_win):
        ...
        cm_df = pd.DataFrame(cm, index=binary_labels, columns=binary_labels)
...
def main() -> None:
    ...
    run_inference(
        best_model, dm.test_samples, dm.label_map or {}, device, output_dir
    )
    print("\nDone.")
