"""
VideoMAE V2 — Full-Video Sliding Window (2-sec window, 1-sec stride)
Single classification head including N/A as a class.
For both Locomotion and Repetitive Motor Movements tasks.
...
TASK_CONFIG: dict[str, dict[str, Any]] = {
    "loco": {
        ...
        "output_dir":  (
            ...
            "/models_output_seeds/clips_h5/vmae2/loco_fullvideo"
        ),
    },
    "rmm": {
        ...
        "output_dir":  (
            "/orcd/data/satra/002/projects/SAILS/vjepa_features"
            "/models_output_seeds/clips_h5/vmae2/rmm_fullvideo"
        ),
    },
}
...
class BBoxCropVideoDataset(Dataset[tuple[torch.Tensor, int]]):
    ...
    def _read_segment(self, s: dict[str, Any]) -> torch.Tensor:
        ...
        frames: list[np.ndarray] = []                      # ← typed list
...
class H5BBoxDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        all_labels = sorted(
            {s["label_str"] for sp in by_split.values() for s in sp}
        )
...
def build_videomae2_vitb(
    ...
    # No type: ignore needed — nn.Module attributes are dynamic
    model.head = nn.Linear(768, num_classes)               # ← removed ignore
    nn.init.trunc_normal_(model.head.weight, std=0.02)     # ← removed ignore
    nn.init.zeros_(model.head.bias)                        # ← removed ignore
...
class VideoMAE2FineTune(pl.LightningModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        result: torch.Tensor = self.model(x)               # ← explicit typed var
        return result                                       # ← no Any leak

    def training_step(                                     # ← no ignore comment
    ...
    def validation_step(                                   # ← no ignore comment
        self, batch: tuple[torch.Tensor, torch.Tensor], _: int
    ) -> torch.Tensor:
        x, y = batch
        logits = self.model(x)
    ...
    def configure_optimizers(self) -> Any:                 # ← Any, not dict[str,Any]
...
# 7. METRICS HELPERS
# ============================================================
def _log_line(lines: list[str], s: str = "") -> None:
    print(s)
    lines.append(s)
...
def compute_full_metrics(
    ...
    top2 = float(top_k_accuracy_score(y_true, y_prob, k=min(2, K)) * 100)
    ...
    for c in range(K):
        if lb.shape[1] > c and lb[:, c].sum() > 0:
            aps.append(float(average_precision_score(lb[:, c], y_prob[:, c])))
        else:
    ...
    per_acc = [
        float((y_pred[y_true == c] == c).mean() * 100)
        if (y_true == c).sum() > 0
        else 0.0
        for c in range(K)
    ]
    ...
    for i, cls in enumerate(all_labels):
        ...
        log(
            f"  {cls:<30} {per_acc[i]:>7.2f} {prec[i]:>7.4f} "
    ...
    log(
        classification_report(
            y_true, y_pred, target_names=all_labels, digits=4, zero_division=0
        )
    )
    log("=" * 60)
        ...
        json.dump(
            {
                ...
                "cohen_kappa": kappa if not np.isnan(kappa) else None,
                "mcc": mcc,
                "mAP": mAP,
                ...
                "per_class": {
                    cls: {
                        ...
                        "recall":    float(rec[i]),
                        "f1":        float(f1[i]),
                        "AP":        aps[i] if not np.isnan(aps[i]) else None,
                        "support":   int(sup[i]),
                    }
                    for i, cls in enumerate(all_labels)
                },
...
def run_inference(
    ...
    prob_cols = [
        f"prob_{c}" for c in all_labels if f"prob_{c}" in valid.columns
    ]
    ...
    if len(valid):
        ...
        y_prob = (
            valid[prob_cols].values
            if prob_cols
            else np.zeros((len(valid), len(all_labels)))
        )
        compute_full_metrics(
            y_true, y_pred, y_prob, all_labels, output_dir, tag="window_level"
        )
    ...
    for vpath, grp in valid.groupby("video_path"):
        avg_probs = (
            grp[prob_cols].mean(axis=0).values
            if prob_cols
        ...
        video_rows.append({
            ...
            **{
                pc: round(float(v), 4)
                for pc, v in zip(prob_cols, avg_probs, strict=False)
            },
        })
    ...
    if len(vid_df):
        ...
        y_prob_v = (
            vid_df[prob_cols].values
            if prob_cols
            else np.zeros((len(vid_df), len(all_labels)))
        )
        compute_full_metrics(
            y_true_v, y_pred_v, y_prob_v, all_labels, output_dir, tag="video_level"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "VideoMAE V2 full-video sliding window fine-tuning "
            "(single head + N/A)"
        )
    )
    ...
    print(
        ...
        f"({WINDOW_SEC}s / {WINDOW_STRIDE}s stride)"
    )
    print(f"{'=' * 60}\n")
    ...
    assert n_classes == cfg["num_classes"], (
        ...
        f"found {n_classes}. Update TASK_CONFIG or check your data."
    )
    ...
    ckpt_cb = pl.callbacks.ModelCheckpoint(
        ...
        save_top_k=2,
        filename=(
            f"vmae2-{args.task}-fullvideo-{{epoch:02d}}-{{val_loss:.3f}}"
        ),
    )
