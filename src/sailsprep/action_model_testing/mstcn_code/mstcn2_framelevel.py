"""MS-TCN2 Frame-Level Action Segmentation.

Adapter for your CSV + pre-extracted features (I3D / VJEPA / R2+1D).

Usage::

    python mstcn2_framelevel.py --feature_type i3d  --label loco  --action train    --seed 42
    python mstcn2_framelevel.py --feature_type vjepa --label rmm  --action train    --seed 123
    python mstcn2_framelevel.py --feature_type i3d  --label loco  --action predict  --seed 42
    python mstcn2_framelevel.py --feature_type i3d  --label loco  --action evaluate --seed 42

Inputs (from CSV columns):
    video_path, label_path, split,
    i3d_full_path, vjepa_full_path, r2plus1d_full_path

Label files (label_path):
    CSV with columns: Frame, <label_col>
    One row per frame at 15 fps.
    NaN/empty frames mapped to "background" automatically.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--feature_type", choices=["i3d", "vjepa", "r2plus1d"], default="i3d"
    )
    p.add_argument(
        "--action", choices=["train", "predict", "evaluate"], default="train"
    )
...
LABEL_CONFIGS: dict[str, dict[str, str]] = {
    "loco": {
        ...
        "output_dir": (
            ...
            "models_output_seeds/mstcn2/loco/"
        ),
    },
    "rmm": {
        ...
        "output_dir": (
            "/orcd/data/satra/002/projects/SAILS/vjepa_features/"
            "models_output_seeds/mstcn2/rmm/"
        ),
    },
}
...
# MS-TCN++ arch
NUM_LAYERS_PG: int = 11  # prediction generation stage layers
NUM_LAYERS_R: int = 10   # refinement stage layers
NUM_R: int = 3            # number of refinement stages
NUM_F_MAPS: int = 64      # feature maps
...
# ============================================================
# MS-TCN++ MODEL (dual dilated PG + separate R layers)
# ============================================================
...
class MultiStageModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # Prediction Generation module with dual dilated pathways.
        self.PG = PredictionGeneration(num_layers_pg, num_f_maps, dim, num_classes)
        # Refinement modules (each uses single dilated pathway).
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """Run PG stage then all refinement stages.

        Args:
            x: Input feature tensor of shape ``(B, D, T)``.
            mask: Binary mask of shape ``(B, 1, T)``.

        Returns:
            Stacked logits of shape ``(num_stages, B, C, T)``.
        """
        # PG stage.
        out = self.PG(x, mask)
        ...
        return outputs  # [num_stages, B, C, T]
...
class PredictionGeneration(nn.Module):
    ...
    def __init__(
        ...
        self.conv_dilated_1: nn.ModuleList = nn.ModuleList()  # decreasing dilation
        self.conv_dilated_2: nn.ModuleList = nn.ModuleList()  # increasing dilation
        ...
        for i in range(num_layers):
            ...
            self.conv_dilated_1.append(
                nn.Conv1d(
                    num_f_maps, num_f_maps, 3, padding=dil_1, dilation=dil_1
                )
            )
            self.conv_dilated_2.append(
                nn.Conv1d(
                    num_f_maps, num_f_maps, 3, padding=dil_2, dilation=dil_2
                )
            )
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """Forward pass through dual-dilated layers.
        ...
        Returns:
            Logit tensor of shape ``(B, num_classes, T)``.
        """
        ...
        return self.conv_out(out) * mask[:, 0:1, :]
...
class Refinement(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """Forward pass through refinement layers.
...
class DilatedResidualLayer(nn.Module):
    """Single dilated residual layer used inside :class:`Refinement`.

    Args:
        dilation: Dilation factor for the dilated convolution.
        in_channels: Number of input channels.
        out_channels: Number of output channels.
    """

    def __init__(
        self, dilation: int, in_channels: int, out_channels: int
    ) -> None:
        super().__init__()
...
class MSTCNLoss(nn.Module):
    """Combined cross-entropy and temporal smoothing loss for MS-TCN2.

    Args:
        num_classes: Number of action classes.
        smoothing_weight: Weight applied to the smoothing term.
        clamp: Upper clamp value for the smoothing loss.
    """
    ...
    def forward(
        ...
        """Compute total loss over all stages.

        Args:
            outputs: Stage logits of shape ``(num_stages, B, C, T)``.
        ...
        for out in outputs:
            ce_loss = self.ce(
                out.transpose(1, 2).contiguous().view(-1, self.num_classes),
                targets.view(-1),
            )
...
def load_feature_file(path: str, expected_dim: int) -> np.ndarray:
    """Load feature file and validate shape is ``(D, T)``.

    Args:
        path: Path to the feature file (.npy, .h5/.hdf5, or .pt).
        expected_dim: Expected feature dimensionality D.

    Returns:
        Float32 array of shape ``(D, T)``.

    Raises:
        ValueError: If the file format is unsupported, the array is not 2-D,
            or neither dimension matches ``expected_dim``.
    """
    ...
    if arr.ndim != 2:
        raise ValueError(
            f"Expected 2D array, got shape {arr.shape} in {path}"
        )

    # Auto-transpose if shape looks like (T, D) instead of (D, T).
    if arr.shape[0] != expected_dim and arr.shape[1] == expected_dim:
        print(
            f"  [shape fix] Transposing {arr.shape} -> "
            f"({arr.shape[1]}, {arr.shape[0]}) for {os.path.basename(path)}"
        )
        arr = arr.T
    elif arr.shape[0] != expected_dim and arr.shape[1] != expected_dim:
        raise ValueError(
            f"Feature dim mismatch in {path}: "
            f"shape={arr.shape}, expected_dim={expected_dim}"
        )
...
def load_label_sequence(
    ...
    """Load a per-frame label sequence from a CSV annotation file.

    Args:
        label_path: Path to the annotation CSV.
        label_col: Column name for the target label.
        label_map: Mapping from label string to integer index.
        num_frames: Total number of frames (length of the output array).

    Returns:
        Integer array of shape ``(num_frames,)`` with class indices.
        Frames without a valid label are assigned the background index.
    """
    try:
        df = pd.read_csv(label_path, encoding="utf-8-sig", keep_default_na=False)
        df.columns = df.columns.str.strip()
    except Exception as exc:  # noqa: BLE001
        print(f"  Warning: cannot read {label_path}: {exc}")
        return np.full(num_frames, label_map[BACKGROUND_LABEL], dtype=np.int64)

    labels = np.full(num_frames, label_map[BACKGROUND_LABEL], dtype=np.int64)
...
def gather_all_labels(
    ...
    """Collect every unique action label across all annotation files.

    Args:
        df_csv: Master split DataFrame with a ``label_path`` column.
        label_col: Target annotation column name.

    Returns:
        Sorted list of unique label strings, always including
        ``BACKGROUND_LABEL``.
    """
...
class FullVideoDataset(Dataset):  # type: ignore[type-arg]
    """Dataset that yields full-video feature sequences with frame labels.

    Args:
        samples: List of row dicts from the split CSV.
        label_col: Annotation column name for the target task.
        label_map: Mapping from label string to integer class index.
        feature_col: CSV column name pointing to the feature file.
        feature_dim: Expected feature dimensionality D.
    """
    ...
    def __getitem__(
        ...
        """Return ``(features, labels, mask, video_id)`` for one video.

        Args:
            idx: Dataset index.

        Returns:
            Tuple of feature tensor ``(D, T)``, label tensor ``(T,)``,
            mask tensor ``(1, T)``, and video basename string.
        """
        ...
        t_len = feat.shape[1]
        labels = load_label_sequence(
            label_path, self.label_col, self.label_map, t_len
        )
        mask = np.ones((1, t_len), dtype=np.float32)
...
def collate_fn(
    ...
    """Pad a batch of variable-length video sequences to the same length.

    Args:
        batch: List of ``(feat, labels, mask, vid_id)`` tuples.

    Returns:
        Tuple of padded ``(feats, labels, masks, vid_ids)`` tensors/list.
        Labels are padded with ``-100`` (ignored by cross-entropy).
    """
    feats, labels, masks, vids = zip(*batch)
    t_max = max(f.shape[1] for f in feats)
    feat_d = feats[0].shape[0]

    feats_pad = torch.zeros(len(feats), feat_d, t_max)
    labels_pad = torch.full((len(feats), t_max), -100, dtype=torch.long)
    masks_pad = torch.zeros(len(feats), 1, t_max)

    for i, (f, lbl, m) in enumerate(zip(feats, labels, masks)):
        t_len = f.shape[1]
        feats_pad[i, :, :t_len] = f
        labels_pad[i, :t_len] = lbl
        masks_pad[i, :, :t_len] = m
...
class MSTCNTrainer:
    """Encapsulates training, inference, and evaluation for MS-TCN2.
    ...
    def _build_model(self) -> MultiStageModel:
        """Instantiate a fresh MultiStageModel from global hyperparameters."""
    ...
    def _seed_dir(self) -> str:
        """Return (and create) the per-seed output subdirectory."""
    ...
    def train(
        ...
        """Train MS-TCN2, saving best weights and a resume checkpoint.

        Args:
            train_samples: List of training sample dicts.
            val_samples: List of validation sample dicts.
            label_col: Annotation column name.
            feature_col: CSV column name for feature file paths.
        ...
        train_ds = FullVideoDataset(
            train_samples, label_col, self.label_map, feature_col, self.feature_dim
        )
        val_ds = FullVideoDataset(
            val_samples, label_col, self.label_map, feature_col, self.feature_dim
        )
        ...
        if os.path.exists(resume_ckpt):
            ...
            # Reload history so the CSV stays continuous.
            if os.path.exists(hist_path):
                history = pd.read_csv(hist_path).to_dict("records")
        else:
            print(
                f"\n*** No resume checkpoint found — "
                f"training from scratch (seed={self.seed})"
            )
        ...
        for epoch in range(start_epoch, NUM_EPOCHS + 1):
            ...
            # Overwrites each time so only 1 extra file on disk.
            torch.save(
                {
                    "epoch": epoch,
        ...
        # ---- Training done: remove resume checkpoint ----
        if os.path.exists(resume_ckpt):
    ...
    def predict(
        ...
        Args:
            ...
            label_col: Annotation column name.
            feature_col: CSV column name for feature file paths.
            ckpt_path: Path to the best-model checkpoint.
            device: Torch device for inference.

        Returns:
            Tuple of ``(all_true, all_pred)`` flat label-string lists.
        """
        seed_dir = self._seed_dir()
        model = self._build_model().to(device)
        ...
        test_ds = FullVideoDataset(
            test_samples, label_col, self.label_map, feature_col, self.feature_dim
        )
        ...
        with torch.no_grad():
            for i in range(len(test_ds)):
                ...
                valid_rows = [
                    r
                    for r in rows
                    if r["video_id"] == vid_id and r["correct"] != -1
                ]
                if valid_rows:
                    video_acc = sum(
                        int(r["correct"]) for r in valid_rows
                    ) / len(valid_rows)
                    print(
                        f"  [{i + 1}/{len(test_ds)}] {vid_id}  acc={video_acc:.3f}"
                    )
    ...
    def evaluate(
        ...
        Args:
            all_true: Flat list of ground-truth label strings.
            all_pred: Flat list of predicted label strings.
            tag: Output file prefix (e.g. ``"test"`` or ``"val"``).

        Returns:
            Dict of scalar and per-class metrics.
        """
        seed_dir = self._seed_dir()
        ...
        per_class_dict = {
            lab: float(f)
            for lab, f in zip(labels_sorted, per_class_f1_arr)
        }
        ...
        with open(report_path, "w") as fh:
            ...
            fh.write(f"Macro Precision: {macro_prec:.4f}\n")
            fh.write(f"Macro Recall:    {macro_rec:.4f}\n\n")
            fh.write(report_str)
            fh.write(f"\n\nConfusion Matrix:\n{cm_df.to_string()}\n")
    ...
    def _save_segment_summary(
        self,
        rows: list[dict[str, object]],
        seed_dir: str,
    ) -> None:
        """Write a run-length-encoded segment summary CSV.

        Args:
            rows: Per-frame prediction rows from :meth:`predict`.
            seed_dir: Directory to write ``test_segment_summary.csv``.
        """
        seg_rows: list[dict[str, object]] = []
        vid_groups: dict[str, list[dict[str, object]]] = defaultdict(list)
        for r in rows:
            vid_groups[str(r["video_id"])].append(r)

        for vid_id, frames in vid_groups.items():
            frames = sorted(frames, key=lambda x: x["frame"])
            for label, group in groupby(
                frames, key=lambda x: x["pred_label"]
            ):
                grp = list(group)
                start = grp[0]["frame"]
                end = grp[-1]["frame"]
...
def aggregate_seeds(
    ...
    Args:
        output_dir: Root directory containing ``seed_<N>`` subdirectories.
        seeds: List of seed values to aggregate.
        tag: Metrics file prefix (e.g. ``"test"``).
    """
    ...
    for key in scalar_keys:
        vals = [float(m[key]) for m in all_metrics]  # type: ignore[arg-type]
        agg_rows.append(
            {
                "metric": key,
    ...
    for m in all_metrics:
        all_classes.update(
            m.get("per_class_f1", {}).keys()  # type: ignore[union-attr]
        )

    for cls in sorted(all_classes):
        vals = [
            float(
                m.get("per_class_f1", {}).get(cls, 0.0)  # type: ignore[union-attr]
            )
            for m in all_metrics
        ]
        agg_rows.append(
    ...
    for _, row in df.iterrows():
        print(
            f"  {row['metric']:25s} : {row['mean']:.4f} ± {row['std']:.4f}  "
...
def main() -> None:
    """Parse CLI arguments and run the chosen action (train/predict/evaluate)."""
    args = parse_args()
    cfg = LABEL_CONFIGS[args.label]

    label_col: str = cfg["label_col"]
    output_dir: str = cfg["output_dir"]
    feature_col: str = FEATURE_COL[args.feature_type]
    feature_dim: int = FEATURE_DIM[args.feature_type]
    seed: int = args.seed
    ...
    print(
        ...
        f"(col={feature_col}, dim={feature_dim})"
    )
    ...
    def row_ok(row: pd.Series) -> bool:  # type: ignore[type-arg]
        return bool(
            os.path.exists(str(row["video_path"]))
            and os.path.exists(str(row["label_path"]))
            and os.path.exists(str(row[feature_col]))
        )
    ...
    train_samples: list[dict[str, object]] = df_csv[splits == "train"].to_dict(
        "records"
    )
    val_samples: list[dict[str, object]] = df_csv[splits == "val"].to_dict(
        "records"
    )
    test_samples: list[dict[str, object]] = df_csv[splits == "test"].to_dict(
        "records"
    )
    print(
    ...
    trainer = MSTCNTrainer(
        num_classes, feature_dim, output_dir, label_map, seed
    )
    ...
    if args.action == "train":
        ...
        ckpt_path = trainer.train(
            train_samples, val_samples, label_col, feature_col, device
        )
        if test_samples:
            all_true, all_pred = trainer.predict(
                test_samples, label_col, feature_col, ckpt_path, device
            )
            trainer.evaluate(all_true, all_pred, tag="test")
        if val_samples:
            all_true_v, all_pred_v = trainer.predict(
                val_samples, label_col, feature_col, ckpt_path, device
            )
