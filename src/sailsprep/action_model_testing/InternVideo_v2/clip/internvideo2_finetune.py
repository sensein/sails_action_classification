...
This module loads the InternVideo2-Stage2_6B vision tower from HuggingFace,
freezes all but the last transformer block, and trains a classification head
on bbox-cropped video clips. Supports multi-seed runs for result spread
analysis.

The critical design constraint: ``AutoModel.from_pretrained`` must be called
inside ``LightningModule.setup()``, **not** inside ``__init__``. With
``torchrun`` DDP each worker process inherits ``CUDA_VISIBLE_DEVICES`` set by
the launcher, and Lightning does not call ``torch.cuda.set_device`` until
*after* ``__init__`` returns. Calling any CUDA op (including HF model loading)
inside ``__init__`` therefore causes ``cudaGetDeviceCount`` Error 101 on ranks
other than rank 0.
...
Typical invocation is via ``job_clip.sh``.
"""
...
#: Master CSV with columns: video_path, label_path, split,
#: interpolated_anno_h5, and feature path columns (unused here).
SPLIT_CSV: str = (
...
#: you have confirmed headroom on your GPU.
BATCH_SIZE: int = 2
...
def find_action_runs(
    ...
    Args:
        ann: Annotation DataFrame loaded from a per-session CSV under
            ``/home/aparnabg/orcd/scratch/app/all_annotation/``.
...
def build_samples(
    ...
    for _, row in split_df.iterrows():
        ...
        if sp not in by_split:
            continue
        if not (
            os.path.exists(vp) and os.path.exists(lp) and os.path.exists(hp)
        ):
            continue
...
class BBoxCropVideoDataset(Dataset):  # type: ignore[type-arg]
    ...
    def _read_segment(self, sample: dict[str, object]) -> torch.Tensor:
        ...
        h5_path = str(sample["h5_path"])
        start_frame = int(sample["start_frame"])  # type: ignore[arg-type]
        end_frame = int(sample["end_frame"])  # type: ignore[arg-type]
        ann_fps = float(sample["ann_fps"])  # type: ignore[arg-type]
        ...
        ann_frames = np.arange(start_frame, end_frame + 1)
...
class H5BBoxDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:  # noqa: ARG002
        ...
        if n_train == 0:
            raise RuntimeError(
                "No training clips built — check split CSV paths."
            )
...
def _freeze_except_last_block(clf: InternVideo2Classifier) -> None:
    ...
    block_module: Optional[nn.Module] = None
    block_attr = ""
    ...
    last_idx = len(block_module) - 1  # type: ignore[arg-type]
    last_prefix = f"backbone.{block_attr}.{last_idx}"
    print(f"  Last block prefix: {last_prefix}  (depth={len(block_module)})")  # type: ignore[arg-type]
...
class InternVideo2FineTune(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model: Optional[InternVideo2Classifier] = None
        # Registered in setup(); declared here so mypy sees the attribute.
        self.class_weights: Optional[torch.Tensor] = None
    ...
    def configure_optimizers(
        self,
    ) -> dict[str, Union[torch.optim.Optimizer, dict[str, object]]]:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "InternVideo2-6B fine-tuning for behavioural action recognition."
        )
    )
    parser.add_argument(
        "--task",
    ...
    output_dir = os.path.join(str(cfg["output_dir"]), f"seed_{args.seed}")
    ...
    assert n_classes == int(cfg["num_classes"]), (
