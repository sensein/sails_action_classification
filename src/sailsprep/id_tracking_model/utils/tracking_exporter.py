...
class TrackingDataExporter:
    ...
    def __init__(self) -> None:
        ...
        self.tracking_data: dict[int, dict[str, Any]] = defaultdict(
            lambda: {
                "start_frame": None,
                "end_frame": None,
                "frames": {},
    ...
    def _process_keypoints(self, keypoints: Any) -> list[list[float]]:
        ...
        processed_kpts: list[list[float]] = []
        for i in range(len(kpts)):
            if len(kpts[i]) >= 3:
                ...
                processed_kpts.append(
                    [float(kpts[i][0]), float(kpts[i][1]), 0.0]
                )

        return processed_kpts

    def finalize_data(self, processing_time: float) -> None:
        ...
        for track_id, track_data in self.tracking_data.items():
            for frame_num, frame_data in track_data["frames"].items():
                ...
                detection = {
                    ...
                    "confidence": float(frame_data["bbox"][4])
                    if len(frame_data["bbox"]) > 4
                    else 1.0,
    ...
    def export_tracks_to_hdf5(
        self, output_dir: str, person_profiles: dict[int, dict[str, Any]]
    ) -> int:
        """
        ...
        for track_id, track_data in self.tracking_data.items():
            ...
            try:
                with h5py.File(h5_filename, "w") as f:
                    ...
                    metadata_grp.attrs["export_timestamp"] = (
                        datetime.now().isoformat()
                    )

                    # Add video metadata
                    if self.video_metadata:
                        metadata_grp.attrs["video_fps"] = self.video_metadata.get(
                            "fps", 0
                        )
                        metadata_grp.attrs["video_width"] = self.video_metadata.get(
                            "width", 0
                        )
                        metadata_grp.attrs["video_height"] = self.video_metadata.get(
                            "height", 0
                        )
                    ...
                    for frame_num in frame_numbers:
                        ...
                        frame_grp.create_dataset(
                            "bbox", data=bbox, compression="gzip", compression_opts=4
                        )
                        ...
                        # Store keypoints
                        keypoints = np.array(
                            frame_data["keypoints"], dtype=np.float32
                        )
                        frame_grp.create_dataset(
                            "keypoints",
                            data=keypoints,
                    ...
                    if profile.get("upper_feature") is not None:
                        upper_emb = np.array(
                            profile["upper_feature"], dtype=np.float32
                        )
                        embeddings_grp.create_dataset(
                            "upper_feature",
                            data=upper_emb,
                    ...
                    if profile.get("lower_feature") is not None:
                        lower_emb = np.array(
                            profile["lower_feature"], dtype=np.float32
                        )
                        embeddings_grp.create_dataset(
                            "lower_feature",
                            data=lower_emb,
                    ...
                    embeddings_grp.attrs["creation_frame"] = profile.get(
                        "creation_frame", -1
                    )
        ...
        if num_skipped > 0:
            print(
                f"Skipped {num_skipped} filtered tracks (removed by post-hoc filtering)"
            )
        return num_exported
...
class TrackingDataCollector:
    ...
    def __init__(
        self, output_path: str | None = None, enable_hdf5: bool = False
    ) -> None:
    ...
    def export_data(
        ...
        person_profiles: dict[int, dict[str, Any]] | None = None,
    ) -> None:
        """
        ...
        if self.enable_hdf5 and person_profiles:
            ...
            hdf5_dir = base_path if base_path.endswith("_hdf5") else base_path + "_hdf5"
