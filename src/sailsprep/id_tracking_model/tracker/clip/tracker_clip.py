...
from sailsprep.feature_processing.tracker.person_tracker import (
...
from sailsprep.feature_processing.utils.tracking_exporter_new import TrackingDataCollector
...
clip_path: str = "sailsprep/id_tracking_model/tracker/clip/CLIP-ReID"
...
def apply_oks_nms(pose_results: list[PoseResult], oks_nms_threshold: float, kpt_threshold: float) -> list[PoseResult]:
    ...
    keep_idx: list[int] = oks_nms(  # type: ignore[no-untyped-call]
        kpts_db=kdb,
...
class RegionExtractor:
    ...
    @staticmethod
    def extract_face_region(frame: np.ndarray, kpts: np.ndarray, bbox: np.ndarray, kpt_threshold: float = 0.3) -> tuple[np.ndarray, tuple[int, int, int, int]] | None:
        ...
        try:
            ...
            if len(head_points) >= 2:
                head_arr = np.array(head_points)
                x_min, y_min = np.min(head_arr, axis=0)
                x_max, y_max = np.max(head_arr, axis=0)
    ...
    @staticmethod
    def extract_upper_body_region(frame: np.ndarray, kpts: np.ndarray, bbox: np.ndarray, pose_type: str, kpt_threshold: float = 0.3) -> tuple[np.ndarray, tuple[int, int, int, int]] | None:
        ...
        try:
            ...
            if neck_point is not None and hip_points:
                ...
                pose_mult: dict[str, tuple[float, float]] = {"sitting": (0.1, 0.75), "lying": (0.2, 0.8), "standing": (0.15, 0.65)}
    ...
    @staticmethod
    def extract_lower_body_region(frame: np.ndarray, kpts: np.ndarray, bbox: np.ndarray, pose_type: str, kpt_threshold: float = 0.3) -> tuple[np.ndarray, tuple[int, int, int, int]] | None:
        ...
        try:
            ...
            if hip_points and ankle_points:
                ...
                x_min, x_max = np.min(all_arr[:, 0]), np.max(all_arr[:, 0])
                padding = 15
                lower_x1, lower_x2 = max(0, int(x_min - padding)), min(frame.shape[1], int(x_max + padding))
            else:
                # Fallback to bbox-based region
                x1, y1, x2, y2 = bbox.astype(int)
                pose_mult_lower: dict[str, float] = {"sitting": 0.6, "standing": 0.55}
                y1_m = pose_mult_lower.get(pose_type, 0.55)
...
class FeatureExtractor:
    ...
    @abstractmethod
    def extract(self, roi: np.ndarray, **kwargs: Any) -> np.ndarray | None:
        pass
...
class CLIPReIDFeatureExtractor:
    ...
    def __init__(self, config: CLIPReIDConfig) -> None:
        """
    ...
    def extract(self, roi: np.ndarray) -> np.ndarray | None:
        """
        ...
        try:
            ...
            with torch.no_grad(), torch.amp.autocast('cuda', enabled=True):  # type: ignore[attr-defined]
                features = self.model(tensor)

            feature_vector: np.ndarray = features.cpu().numpy()[0]
            norm = np.linalg.norm(feature_vector)
            if norm > 0:
                result: np.ndarray = feature_vector / norm
                return result
...
class FeatureExtractionModule:
    ...
    def __init__(self, config: FeatureConfig, clip_config: CLIPReIDConfig, kpt_threshold: float = 0.3) -> None:
        self.config = config
        self.kpt_threshold = kpt_threshold
        self.clip_config = clip_config

        self.body_extractor: CLIPReIDFeatureExtractor | None = None
        ...
        self.face_extractor: CLIPReIDFeatureExtractor | FaceFeatureExtractor | None = None
    ...
    def extract_features(self, frame: np.ndarray, pose_results: list[PoseResult], frame_count: int) -> list[dict[str, Any]]:
    ...
    def _create_detection(self, frame: np.ndarray, pose: PoseResult, frame_count: int) -> dict[str, Any] | None:
    ...
    def _determine_pose_type(self, kpts: np.ndarray) -> str:
...
class DetectionValidator:
    ...
    def __init__(self) -> None:
        ...
        self.texture_history: dict[int, deque[dict[str, float]]] = {}
        self.size_history: dict[int, deque[float]] = {}
    ...
    def is_real_person(self, track_id: int, bbox: np.ndarray, frame: np.ndarray,
        ...
        if roi.shape[0] < 20 or roi.shape[1] < 20:
            return False  # Too small to analyze (remove small detection)
        ...
        if len(self.size_history[track_id]) >= min_frames:
            ...
            if avg_size > 0:
                cv_val = size_std / avg_size
                if cv_val > 0.4:  # More than 40% variation
                    return False

        return True

    def update_motion(self, track_id: int, motion_score: float) -> None:
...
class TrackingModule:
    ...
    def __init__(self, config: ProcessingConfig) -> None:
        ...
        self.person_profiles: dict[int, dict[str, Any]] = {}

        # Candidate tracking
        self.candidate_tracks: dict[int, dict[str, Any]] = {}
        ...
        self.invalid_track_ids: set[int] = set()  # For the tracks marked as photos/screens

    def _calculate_motion_score(self, track: dict[str, Any]) -> float:
    ...
    def update(self, detections: list[dict[str, Any]], frame: np.ndarray) -> dict[int, int]:
    ...
    def _match_with_motion(self, detections: list[dict[str, Any]], camera_motion: tuple[Any, ...],
                                    iou_thresh: float, center_weight: float,
                                    motion_conf_thresh: float, all_tracks: dict[int, dict[str, Any]]) -> dict[int, int]:
        """
        ...
        track_predictions: dict[int, Any] = {}
        for track_id in track_ids:
            track = all_tracks[track_id]
        ...
        for det_idx, track_idx in zip(det_indices, track_indices, strict=False):
    ...
    def _match_with_appearance(self, unmatched_detections: list[tuple[int, dict[str, Any]]],
                                        all_tracks: dict[int, dict[str, Any]]) -> dict[int, tuple[int, str]]:
        """
        ...
        match_types: dict[tuple[int, int], tuple[str, float]] = {}

        for i, (_det_idx, detection) in enumerate(unmatched_detections):
    ...
    def _match_with_lost_tracks(self, unmatched_detections: list[tuple[int, dict[str, Any]]]) -> dict[int, tuple[int, str]]:
        """
        ...
        for i, (_det_idx, detection) in enumerate(unmatched_detections):
            for j, (track_id, _track) in enumerate(eligible_tracks):
                profile = self.person_profiles[track_id]
                similarity, match_type = self._compute_person_similarity(detection, profile)
    ...
    def _compute_person_similarity(self, detection: dict[str, Any], profile: dict[str, Any]) -> tuple[float, str]:
        """
        ...
        for feat_cfg in feature_configs:
            det_feat = detection.get(feat_cfg['feat'])
            prof_feat = profile.get(feat_cfg['feat'])
            ...
            if det_feat is not None and prof_feat is not None:
                ...
                threshold: float = feat_cfg['threshold']
                if not face_available and feat_cfg['name'] != 'face':
                    threshold *= 0.90  # 10% more lenient for body-only matching

                if sim > threshold:
                    similarities.append(sim)
                    matching_components.append(feat_cfg['name'])
                    weights.append(feat_cfg['weight'])

        if similarities and weights:
            # Normalize weights (critical when some features missing)
            total_weight = sum(weights)
            normalized_weights = [w / total_weight for w in weights]
            combined_sim = sum(s * w for s, w in zip(similarities, normalized_weights, strict=False))
    ...
    def _compute_feature_similarity(self, feat1: np.ndarray, feat2: np.ndarray) -> float:
        ...
        try:
            similarity: float = cosine_similarity(feat1.reshape(1, -1), feat2.reshape(1, -1))[0, 0]
            return max(0.0, similarity)
        except Exception as e:
            print(f"Error computing feature similarity: {e}")
            return 0.0

    def _update_person_profile(self, profile: dict[str, Any], detection: dict[str, Any]) -> None:
        """
    ...
    def _handle_lost_tracks_with_candidates(self, final_matches: dict[int, int]) -> None:
    ...
    def _create_candidate_track(self, detection: dict[str, Any]) -> int | None:
    ...
    def _update_candidate_track(self, detection: dict[str, Any], candidate_id: int) -> None:
        """
    ...
    def _promote_confirmed_candidates(self) -> None:
        """
        ...
        for candidate_id in candidates_to_promote:
            ...
            if len(self.person_profiles) > 0:
                ...
                for _existing_id, profile in self.person_profiles.items():
                    sim, match_type = self._compute_person_similarity(last_det, profile)
                    #if sim > 0.5:  # Potential duplicate
                    #    print(f"  High similarity ({sim:.2f}, {match_type}) with Track {_existing_id}")
            # Assign PERMANENT ID
    ...
    def _cleanup_failed_candidates(self) -> None:
        """
        ...
        for candidate_id, track in self.candidate_tracks.items():
            ...
            if track['lost_frames'] > 10 or age > 30 and track['confirmation_count'] < self.confirmation_frames_required:
                candidates_to_remove.append(candidate_id)

        for candidate_id in candidates_to_remove:
            del self.candidate_tracks[candidate_id]

    def _check_temporal_overlap(self, track1: dict[str, Any], track2: dict[str, Any]) -> bool:
        """
    ...
    def _merge_non_overlapping_tracks(self, all_tracks: dict[int, dict[str, Any]],
    ...
    def _apply_track_merging(self, all_tracks: dict[int, dict[str, Any]], merge_map: dict[int, int]) -> None:
        """
    ...
    def _check_appearance_consistency(self, motion_matches: dict[int, int], detections: list[dict[str, Any]]) -> list[int]:
    ...
    def _check_geometry_consistency(self, motion_matches: dict[int, int], detections: list[dict[str, Any]]) -> list[int]:
    ...
    def _ensure_track_geometry(self, track: dict[str, Any], bbox: np.ndarray) -> None:
    ...
    def _update_track_geometry(self, track: dict[str, Any], detection: dict[str, Any], *, clean: bool = True) -> None:
...
class VisualizationModule:
    ...
    def __init__(self, config: VisualizationConfig, kpt_threshold: float = 0.3) -> None:
        self.config = config
        self.kpt_threshold = kpt_threshold

    def draw_tracking_results(self, frame: np.ndarray, pose_results: list[PoseResult],
                        person_assignments: dict[int, int], active_tracks: dict[int, dict[str, Any]]) -> np.ndarray:
        ...
        if self.config.enable_pose_drawing:
            for pose in pose_results:  # Use pose_results directly
                ...
                for _i, (kpt, score) in enumerate(zip(keypoints, keypoint_scores, strict=False)):
...
class MultiPersonTrackingPipeline:
    ...
    def __init__(self, config: PipelineConfig, data_collector: TrackingDataCollector | None = None, batch_signal_handler: Any = None) -> None:
        self.config = config
        ...
        self.visualization_module: VisualizationModule | None = (
            VisualizationModule(
        ...
        # Data export
        self.data_collector: TrackingDataCollector | None = (
            (data_collector or TrackingDataCollector(enable_hdf5=config.export.export_hdf5))
            if config.export.enable_export else None
        )

        # Performance tracking
        self.timing_stats: defaultdict[str, list[float]] = defaultdict(list)
    ...
    def _signal_handler(self, signum: int, frame: Any) -> None:
        ...
        if self.data_collector and self.global_start_time is not None:
    ...
    def _check_extremely_stationary_track(self, track: dict[str, Any], threshold_pixels: float = 2.0, min_keypoint_confidence: float = 0.6) -> bool:
        """
        ...
        avg_kpt_confidence = float(np.mean(keypoint_confidences))
        avg_visible_kpts = float(np.mean(visible_keypoint_counts))
        ...
        is_artifact = bool(is_stationary) and (low_confidence or few_keypoints)
    ...
    def _filter_artifacts(self, all_tracks: dict[int, dict[str, Any]]) -> tuple[dict[int, dict[str, Any]], list[int]]:
        """
    ...
    def process_video(self, input_path: str, output_path: str, segment: tuple[int, int] | None = None) -> None:
        ...
        minimal_frame_data: dict[int, dict[str, Any]] = {}  # Store assignments for visualization ONLY
        frame_ix = -1
        ...
        candidate_to_permanent: dict[int, int] = {}
        ...
        for frame_idx in minimal_frame_data:
            det_to_track: dict[int, int] = minimal_frame_data[frame_idx]['det_to_track']
        ...
        proc: subprocess.Popen[bytes] = subprocess.Popen(ffmpeg_cmd, stdin=subprocess.PIPE,
                               stderr=subprocess.DEVNULL)

        # Combine active + lost for visualization
        all_final_tracks: dict[int, dict[str, Any]] = {
            **self.tracking_module.active_tracks,
            **self.tracking_module.lost_tracks,
            **self.tracking_module.candidate_tracks,
        }
        ...
        with tqdm(total=total_frames, desc="Rendering") as pbar:
            while cap.isOpened():
                ...
                if (frame_ix in minimal_frame_data and
                    ...
                    assignments: dict[int, int] = minimal_frame_data[frame_ix]['det_to_track']
    ...
    def _remap_collector_ids(self, full_remap: dict[int, int], removed_ids: set[int]) -> None:
        """
        ...
        remapping_stats: dict[str, int] = {'remapped': 0, 'removed': 0, 'kept': 0}
        ...
        print("  Data collector remapping:")
    ...
    def reset_for_next_video(self) -> None:
