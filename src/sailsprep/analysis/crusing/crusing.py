...
OUTPUT_DIR = "/orcd/data/satra/002/projects/SAILS/action_outputs_features/analysis/crusing/v3"
FIG_DIR    = os.path.join(OUTPUT_DIR, "figures")
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(FIG_DIR,    exist_ok=True)

WALKING_FEATURES_CSV = "/orcd/data/satra/002/projects/SAILS/action_outputs_features/analysis/walking/v3/child_level_features.csv"
...
CRUISING_FEAT_SHORT = {
    'trunk_lateral_lean_mean':         'Trunk Lateral Lean (mean)',
    'trunk_lateral_lean_std':          'Trunk Lateral Lean (std)',
    'trunk_lateral_lean_amplitude':    'Trunk Lateral Lean (amp)',
    'trunk_lateral_lean_abs_mean':     'Trunk Lateral Lean (abs)',
    'support_arm_reach_L_mean':        'Support Arm Reach L',
    'support_arm_reach_R_mean':        'Support Arm Reach R',
    'support_arm_reach_asym':          'Arm Reach Asymmetry',
    'wrist_support_asym':              'Wrist Support Asym',
    'shoulder_x_amplitude':            'Shoulder X Amplitude',
    'shoulder_x_std':                  'Shoulder X Std',
    'shoulder_y_amplitude':            'Shoulder Y Amplitude',
    'shoulder_y_std':                  'Shoulder Y Std',
    'hip_x_amplitude':                 'Hip X Amplitude',
    'hip_x_std':                       'Hip X Std',
    'hip_x_vel_mean':                  'Hip X Velocity',
    'hip_x_vel_std':                   'Hip X Vel Std',
    'hip_x_sparc':                     'Hip X SPARC',
    'hip_x_jerk_mean':                 'Hip X Jerk',
    'hip_x_dom_freq':                  'Hip X Dom Freq',
    'hip_x_band_power':                'Hip X Band Power',
    'hip_x_spectral_entropy':          'Hip X Spectral Entropy',
    'hip_y_std':                       'Hip Y Std (vert stability)',
    'hip_y_amplitude':                 'Hip Y Amplitude',
    'ankle_x_L_amplitude':             'Ankle L X Amplitude',
    'ankle_x_R_amplitude':             'Ankle R X Amplitude',
    'ankle_x_L_std':                   'Ankle L X Std',
    'ankle_x_R_std':                   'Ankle R X Std',
    'ankle_x_L_sparc':                 'Ankle L X SPARC',
    'ankle_x_R_sparc':                 'Ankle R X SPARC',
    'ankle_x_L_jerk_mean':             'Ankle L X Jerk',
    'ankle_x_R_jerk_mean':             'Ankle R X Jerk',
    'ankle_x_L_dom_freq':              'Ankle L X Dom Freq',
    'ankle_x_R_dom_freq':              'Ankle R X Dom Freq',
    'ankle_x_L_band_power':            'Ankle L X Band Power',
    'ankle_x_R_band_power':            'Ankle R X Band Power',
    'lateral_step_asym':               'Lateral Step Asymmetry',
    'ankle_x_lr_corr':                 'Ankle X L-R Correlation',
    'step_width_mean':                 'Step Width (mean)',
    'step_width_std':                  'Step Width (std)',
    'step_width_cv':                   'Step Width CV',
    'lateral_step_duration_mean':      'Lateral Step Duration',
    'lateral_step_duration_cv':        'Lateral Step Duration CV',
    'lateral_cadence':                 'Lateral Cadence',
    'lateral_step_regularity':         'Lateral Step Regularity',
    'wrist_x_L_amplitude':             'Wrist L X Amplitude',
    'wrist_x_R_amplitude':             'Wrist R X Amplitude',
    'wrist_y_L_amplitude':             'Wrist L Y Amplitude',
    'wrist_y_R_amplitude':             'Wrist R Y Amplitude',
    'wrist_x_L_sparc':                 'Wrist L X SPARC',
    'wrist_x_R_sparc':                 'Wrist R X SPARC',
    'wrist_x_lr_asym':                 'Wrist X L-R Asymmetry',
    'wrist_x_lr_corr':                 'Wrist X L-R Correlation',
    'elbow_angle_L_mean':              'Elbow L Angle (mean)',
    'elbow_angle_R_mean':              'Elbow R Angle (mean)',
    'elbow_angle_L_std':               'Elbow L Angle (std)',
    'elbow_angle_R_std':               'Elbow R Angle (std)',
    'elbow_angle_asym':                'Elbow Angle Asymmetry',
    'head_x_amplitude':                'Head X Amplitude',
    'head_x_std':                      'Head X Std',
    'head_y_amplitude':                'Head Y Amplitude',
    'trunk_inclination_mean':          'Trunk Inclination (mean)',
    'trunk_inclination_std':           'Trunk Inclination (std)',
    'trunk_forward_lean_mean':         'Trunk Forward Lean',
    'knee_angle_L_mean':               'Knee L Angle (mean)',
    'knee_angle_R_mean':               'Knee R Angle (mean)',
    'knee_angle_L_std':                'Knee L Angle (std)',
    'knee_angle_R_std':                'Knee R Angle (std)',
    'knee_angle_L_range':              'Knee L ROM',
    'knee_angle_R_range':              'Knee R ROM',
    'knee_angle_L_cv':                 'Knee L Angle CV',
    'knee_angle_R_cv':                 'Knee R Angle CV',
    'knee_angle_asym':                 'Knee Angle Asymmetry',
    'hip_angle_L_mean':                'Hip L Angle (mean)',
    'hip_angle_R_mean':                'Hip R Angle (mean)',
    'hip_angle_L_range':               'Hip L ROM',
    'hip_angle_R_range':               'Hip R ROM',
    'ankle_y_L_amplitude':             'Ankle L Y Amplitude',
    'ankle_y_R_amplitude':             'Ankle R Y Amplitude',
    'ankle_y_lr_asym':                 'Ankle Y L-R Asymmetry',
    'stride_knee_rom_mean':            'Stride Knee ROM (mean)',
    'stride_knee_rom_cv':              'Stride Knee ROM CV',
}

FEAT_LABEL = lambda f: CRUISING_FEAT_SHORT.get(f, f.replace('_', ' '))
...
def detect_lateral_steps(ankle_x, fps=15.0, min_distance=5):
    """Detect lateral cruising steps from ankle X trajectory."""
    if len(ankle_x) < 20: return []
    try:
        sm = butter_lp(ankle_x, cutoff=3.0, fs=fps)
    except: sm = ankle_x
    std_val = np.std(sm)
    if std_val < 1e-8: return []
    peaks,   _ = find_peaks( sm, distance=min_distance, prominence=std_val*0.3)
    troughs, _ = find_peaks(-sm, distance=min_distance, prominence=std_val*0.3)
    events     = sorted(list(peaks)+list(troughs))
    if len(events) < 2: return []
    return [(int(events[i]), int(events[i+1]))
...
# ═══════════════════════════════════════════════════════════════════
# PART 1: CRUISING FEATURE EXTRACTION
# ═══════════════════════════════════════════════════════════════════
hr("PART 1: CRUISING FEATURE EXTRACTION")

def extract_cruising_features(pose_frames, frame_indices, fps=FPS):
    """
    Extract cruising-specific kinematic features.
    Primary axis: X (lateral).  Upper body is central, not secondary.
    """
    ankle_x_L, ankle_x_R = [], []
    ankle_y_L, ankle_y_R = [], []
    hip_x, hip_y         = [], []
    ...
    for fi in frame_indices:
        ...
        if la:
            ankle_x_L.append(la['x']/scale); ankle_y_L.append(la['y']/scale)
            conf_vals.append(la['confidence'])
        if ra:
            ankle_x_R.append(ra['x']/scale); ankle_y_R.append(ra['y']/scale)
            conf_vals.append(ra['confidence'])

        hip_x_this = hip_y_this = None
        if lh and rh:
            hip_x_this = (lh['x']+rh['x'])/2/scale
            hip_y_this = (lh['y']+rh['y'])/2/scale
            conf_vals.append((lh['confidence']+rh['confidence'])/2)
        elif lh:  hip_x_this=lh['x']/scale; hip_y_this=lh['y']/scale
        elif rh:  hip_x_this=rh['x']/scale; hip_y_this=rh['y']/scale
        if hip_x_this is not None:
            hip_x.append(hip_x_this); hip_y.append(hip_y_this)

        ls = get_kp(fd, KP['L_shoulder']); rs = get_kp(fd, KP['R_shoulder'])
        sh_x_this = sh_y_this = None
        if ls and rs:
            sh_x_this = (ls['x']+rs['x'])/2/scale
            sh_y_this = (ls['y']+rs['y'])/2/scale
            shoulder_x.append(sh_x_this); shoulder_y.append(sh_y_this)
            conf_vals.append(ls['confidence'])
        if sh_x_this is not None and hip_x_this is not None:
            trunk_lateral_lean.append(sh_x_this - hip_x_this)
        if sh_y_this is not None and hip_y_this is not None:
            trunk_forward_lean.append(sh_y_this - hip_y_this)

        lw = get_kp(fd, KP['L_wrist']); rw = get_kp(fd, KP['R_wrist'])
        if lw:
            wrist_x_L.append(lw['x']/scale); wrist_y_L.append(lw['y']/scale)
            if hip_x_this is not None:
                wrist_hip_offset_L.append(lw['x']/scale - hip_x_this)
        if rw:
            wrist_x_R.append(rw['x']/scale); wrist_y_R.append(rw['y']/scale)
            if hip_x_this is not None:
                wrist_hip_offset_R.append(rw['x']/scale - hip_x_this)

        ns = get_kp(fd, KP['nose'])
        if ns: nose_x.append(ns['x']/scale); nose_y.append(ns['y']/scale)
    ...
    rec = {
        ...
        'torso_cv':       float(np.std(torso_lens)/np.mean(torso_lens))
                          if len(torso_lens)>3 else np.nan,
    }
    ...
    # 1. Lateral hip motion (primary)
    if len(hip_x) >= 8: traj_feats(hip_x, 'hip_x')
    if len(hip_y) >= 8: traj_feats(hip_y, 'hip_y'); rec['hip_y_std']=float(np.std(hip_y))

    # 2. Ankle X (lateral step excursion per foot)
    traj_feats(ankle_x_L, 'ankle_x_L'); traj_feats(ankle_x_R, 'ankle_x_R')
    if len(ankle_y_L)>=8: rec['ankle_y_L_amplitude']=float(np.ptp(ankle_y_L))
    if len(ankle_y_R)>=8: rec['ankle_y_R_amplitude']=float(np.ptp(ankle_y_R))
    if ankle_y_L and ankle_y_R:
        rec['ankle_y_lr_asym']=float(abs(np.ptp(ankle_y_L)-np.ptp(ankle_y_R))/(np.ptp(ankle_y_L)+np.ptp(ankle_y_R)+1e-8))
    if ankle_x_L and ankle_x_R:
        amp_l=float(np.ptp(ankle_x_L)); amp_r=float(np.ptp(ankle_x_R))
        rec['lateral_step_asym']=float(abs(amp_l-amp_r)/(amp_l+amp_r+1e-8))
        ml=min(len(ankle_x_L),len(ankle_x_R))
        if ml>=5: rec['ankle_x_lr_corr']=float(np.corrcoef(ankle_x_L[:ml],ankle_x_R[:ml])[0,1])

    # 3. Step width (ankle Y separation)
    if len(ankle_y_L)>=3 and len(ankle_y_R)>=3:
        ml=min(len(ankle_y_L),len(ankle_y_R))
        sw=np.abs(np.array(ankle_y_L[:ml])-np.array(ankle_y_R[:ml]))
        rec['step_width_mean']=float(np.mean(sw)); rec['step_width_std']=float(np.std(sw))
        rec['step_width_cv']=float(np.std(sw)/(np.mean(sw)+1e-8))

    # 4. Lateral step timing
    primary_ax = ankle_x_L if len(ankle_x_L)>=len(ankle_x_R) else ankle_x_R
    if len(primary_ax) >= 20:
        try:
            sm_ax = butter_lp(np.array(primary_ax), cutoff=3.0, fs=fps)
            steps = detect_lateral_steps(sm_ax, fps=fps)
        except: steps = []
        if len(steps) >= 2:
            durs = [(e-s)/fps for s,e in steps]
            rec['lateral_step_duration_mean'] = float(np.mean(durs))
            rec['lateral_step_duration_cv']   = float(np.std(durs)/(np.mean(durs)+1e-8))
            rec['lateral_cadence']             = float(len(steps)/(len(primary_ax)/fps))
            rec['lateral_step_regularity']     = float(1.0-rec['lateral_step_duration_cv'])
            knee_list = knee_angles_L if len(knee_angles_L)>=len(knee_angles_R) else knee_angles_R
            stride_roms = []
            for cs,ce in steps:
                kslice=[knee_list[j] for j in range(min(len(knee_list),ce+1)) if cs<=j<=ce]
    ...
    if len(trunk_lateral_lean)>=5:
        tll=np.array(trunk_lateral_lean)
        rec['trunk_lateral_lean_mean']      = float(np.mean(tll))
        rec['trunk_lateral_lean_std']       = float(np.std(tll))
        rec['trunk_lateral_lean_amplitude'] = float(np.ptp(tll))
        rec['trunk_lateral_lean_abs_mean']  = float(np.mean(np.abs(tll)))
    if len(trunk_forward_lean)>=5:
        rec['trunk_forward_lean_mean'] = float(np.mean(trunk_forward_lean))
        rec['trunk_forward_lean_std']  = float(np.std(trunk_forward_lean))

    # 6. Shoulder girdle
    if len(shoulder_x)>=8: traj_feats(shoulder_x,'shoulder_x')
    if len(shoulder_y)>=8: traj_feats(shoulder_y,'shoulder_y')

    # 7. Wrist features
    traj_feats(wrist_x_L,'wrist_x_L'); traj_feats(wrist_x_R,'wrist_x_R')
    if len(wrist_y_L)>=8: rec['wrist_y_L_amplitude']=float(np.ptp(wrist_y_L))
    if len(wrist_y_R)>=8: rec['wrist_y_R_amplitude']=float(np.ptp(wrist_y_R))
    if wrist_x_L and wrist_x_R:
        amp_l=float(np.ptp(wrist_x_L)); amp_r=float(np.ptp(wrist_x_R))
        rec['wrist_x_lr_asym']=float(abs(amp_l-amp_r)/(amp_l+amp_r+1e-8))
        ml=min(len(wrist_x_L),len(wrist_x_R))
        if ml>=5: rec['wrist_x_lr_corr']=float(np.corrcoef(wrist_x_L[:ml],wrist_x_R[:ml])[0,1])
    if len(wrist_hip_offset_L)>=5:
        rec['support_arm_reach_L_mean']=float(np.mean(wrist_hip_offset_L))
        rec['support_arm_reach_L_std'] =float(np.std(wrist_hip_offset_L))
        rec['support_arm_reach_L_amp'] =float(np.ptp(wrist_hip_offset_L))
    if len(wrist_hip_offset_R)>=5:
        rec['support_arm_reach_R_mean']=float(np.mean(wrist_hip_offset_R))
        rec['support_arm_reach_R_std'] =float(np.std(wrist_hip_offset_R))
        rec['support_arm_reach_R_amp'] =float(np.ptp(wrist_hip_offset_R))
    if 'support_arm_reach_L_mean' in rec and 'support_arm_reach_R_mean' in rec:
        l_m=abs(rec['support_arm_reach_L_mean']); r_m=abs(rec['support_arm_reach_R_mean'])
        rec['support_arm_reach_asym']=float(abs(l_m-r_m)/(l_m+r_m+1e-8))
        rec['wrist_support_asym']    =rec['support_arm_reach_asym']

    # 8. Head motion
    if len(nose_x)>=8: rec['head_x_amplitude']=float(np.ptp(nose_x)); rec['head_x_std']=float(np.std(nose_x))
    if len(nose_y)>=8: rec['head_y_amplitude']=float(np.ptp(nose_y)); rec['head_y_std']=float(np.std(nose_y))
...
# ── Extraction loop ──────────────────────────────────────────────
all_features=[]
n_ok=n_fail_label=n_fail_hrnet=n_fail_cruise=n_fail_kp=n_skip_conf=n_skip_cam=0
...
for proc_idx, (_, row) in enumerate(df_main.iterrows()):
    ...
    cruise_mask=None
    if 'Locomotion' in anno.columns:
        anno['Locomotion']=anno['Locomotion'].astype(str)
        cruise_mask=anno['Locomotion'].str.lower().str.contains('cruis',na=False)
    else:
        for col in anno.select_dtypes(include='object').columns:
            m=anno[col].astype(str).str.lower().str.contains('cruis',na=False)
            if m.any(): cruise_mask=m; break

    if cruise_mask is None or cruise_mask.sum()<MIN_SEG_FRAMES: n_fail_cruise+=1; continue
    ...
    cruise_idx=anno.index[cruise_mask].tolist()
    segments=[]; seg_s=cruise_idx[0]
    for i in range(1,len(cruise_idx)):
        if cruise_idx[i]-cruise_idx[i-1]>GAP_TOL:
            if cruise_idx[i-1]-seg_s>=MIN_SEG_FRAMES: segments.append((seg_s,cruise_idx[i-1]))
            seg_s=cruise_idx[i]
    if cruise_idx[-1]-seg_s>=MIN_SEG_FRAMES: segments.append((seg_s,cruise_idx[-1]))
    ...
    for seg_s,seg_e in segments:
        ...
        for fi in fidx:
            ...
            if fk in frames:
                fd=frames[fk]
                for kp_name in ['L_ankle','R_ankle','L_knee','R_knee',
                                 'L_hip','R_hip','L_shoulder','R_shoulder']:
                    kp=fd.get(KP.get(kp_name,''))
                    if isinstance(kp,dict): conf_list.append(kp.get('confidence',0))
        ...
        feats=extract_cruising_features(frames,fidx,fps=ann_fps)
...
print(f"\nExtraction: OK={n_ok}  NoLabel={n_fail_label}  NoHRNet={n_fail_hrnet}  "
      f"NoCruise={n_fail_cruise}  LowConf={n_skip_conf}  BadCam={n_skip_cam}  NoKP={n_fail_kp}")

if n_ok==0:
    print("ERROR: No cruising segments extracted."); import sys; sys.exit(1)
...
PRIMARY_FEATS=[f for f in ALL_FEAT_COLS if any(
    tok in f for tok in [
        ...
        'shoulder','trunk','wrist','elbow',
        'knee','step_width','lateral_step','lateral_cadence',
        'support_arm','head_x','head_y','sparc','jerk',
        'dom_freq','band_power','spectral','vel_mean',
        'asym','corr','lean','reach','cadence',
        'regularity','stride_knee',
...
def compute_icc(clip_df, feat_cols):
    """Intraclass correlation — reliability of cruising features within child."""
    records=[]
...
def run_lme_kr(clip_df, feat_cols, subset_label='ALL',
    ...
    Random slope (1+age_mo_c|pid) when sessions >= MIN_SESSIONS_FOR_SLOPE.
    Group×Age interaction included by default.
...
def make_consensus(results_dict, feat_cols, threshold=0.05):
    ...
    # safe DataFrame lookup avoids the ambiguous-bool ValueError
    lme_df=next((results_dict.get(k) for k in ('LME_KR','LME_noKR','LME')
                 if results_dict.get(k) is not None and not results_dict[k].empty), None)
    if lme_df is not None and 'cohens_d' in lme_df.columns:
...
for band in AGE_BANDS.keys():
    ...
    if min_n<MIN_N_PER_GROUP:
        ...
        if n_asd>=3 and n_nasd>=3:
            ...
            band_dict={k:v for k,v in {'ChildPerm':band_perm,'PseudobulkMW':band_mw}.items()
            ...
            if band_dict:
                ...
                band_cons.to_csv(os.path.join(OUTPUT_DIR,
                    f'stats_{band.replace("-","_")}_exploratory.csv'),index=False)
        continue
    ...
    all_band_results.append(band_cons)
    band_cons.to_csv(os.path.join(OUTPUT_DIR,
        f'stats_{band.replace("-","_")}_consensus.csv'),index=False)
    for _,r in band_cons[band_cons['n_methods_sig']>0].head(3).iterrows():
...
def run_consistency_gate_bands(child_df, feat_cols, sig_feats):
    """
    Cruising consistency gate: checks effect direction is consistent
    across the three age bands (same logic as walking v3).
    """
    band_mwu={}
    for band in AGE_BANDS.keys():
        sub=child_df[child_df['age_band']==band]
        if sub[sub['Group']=='ASD']['pid'].nunique()<3: continue
        if sub[sub['Group']=='Non-ASD']['pid'].nunique()<3: continue
        recs=[]
        for feat in feat_cols:
...
# ── Step 10: Additional analyses (Spearman, within-ASD) ─────────
...
for grp in GROUPS:
    ...
    for feat in PRIMARY_FEATS:
        ...
        r_,p_=stats.spearmanr(vals['age_mo'],vals[feat])
        sp_recs.append({'Group':grp,'feature':feat,'spearman_r':r_,'p_raw':p_,'n':len(vals)})
if sp_recs:
    sp_df=fdr_annotate(pd.DataFrame(sp_recs),'p_raw')
    sp_df.to_csv(os.path.join(OUTPUT_DIR,'stats_age_spearman.csv'),index=False)
    sig_sp=sp_df[sp_df['sig_raw05']].sort_values('p_raw')
    print(f"  Sig age correlations: {len(sig_sp)}")
    for _,row in sig_sp.head(8).iterrows():
        print(f"    {row['Group']:<9} {row['feature']:<48} r={row['spearman_r']:.3f} p={row['p_raw']:.4f}")

print("\n--- Step 10b: Within-ASD early vs late ---")
asd_early=feat_df[(feat_df['Group']=='ASD')&(feat_df['age_band']=='11-18mo')]
asd_late =feat_df[(feat_df['Group']=='ASD')&(feat_df['age_band']=='32-38mo')]
if len(asd_early)>=5 and len(asd_late)>=5:
    asd_traj_recs=[]
    for feat in PRIMARY_FEATS:
        a_=asd_early[feat].dropna().values; b_=asd_late[feat].dropna().values
        if len(a_)<3 or len(b_)<3: continue
        stat_,p_=stats.mannwhitneyu(a_,b_,alternative='two-sided')
        d_=cohen_d(a_,b_)
        asd_traj_recs.append({'feature':feat,'early_median':float(np.median(a_)),
                               'late_median':float(np.median(b_)),'mw_p':float(p_),'cohens_d':d_})
    if asd_traj_recs:
        asd_traj_df=fdr_annotate(pd.DataFrame(asd_traj_recs),'mw_p')
        asd_traj_df.to_csv(os.path.join(OUTPUT_DIR,'stats_within_ASD_trajectory.csv'),index=False)
        print(f"  Within-ASD traj: sig_raw={asd_traj_df['sig_raw05'].sum()} FDR={asd_traj_df['sig_fdr05'].sum()}")
...
def run_loso_child(cdf, feat_cols, clf_name='LR', n_perm=500, seed=42, subset_name=''):
    ...
    print(f"  [{subset_name} {clf_name}] AUC={auc:.3f}  AP={ap:.3f}  p_perm={p_perm:.4f}")
    fi_df=pd.DataFrame()
    if clf_name=='RF':
...
# ═══════════════════════════════════════════════════════════════════
# PART 5: CRUISING vs WALKING COMPARISON
# ═══════════════════════════════════════════════════════════════════
hr("PART 5: CRUISING vs WALKING COMPARISON")

walk_child_df=None; cruise_walk_comparison=pd.DataFrame()
if WALKING_FEATURES_CSV and os.path.isfile(WALKING_FEATURES_CSV):
    try:
        walk_child_df=pd.read_csv(WALKING_FEATURES_CSV)
        print(f"  Walking CSV loaded: {len(walk_child_df)} children")
    except Exception as e:
        print(f"  Could not load walking CSV: {e}")

SHARED_FEATS=['knee_angle_L_mean','knee_angle_R_mean','knee_angle_L_std','knee_angle_R_std',
              'knee_angle_L_range','knee_angle_R_range','hip_angle_L_mean','hip_angle_R_mean',
              'elbow_angle_L_mean','elbow_angle_R_mean','trunk_inclination_mean','trunk_inclination_std']

if walk_child_df is not None:
    common=[f for f in SHARED_FEATS if f in child_df.columns and f in walk_child_df.columns]
    if common:
        rows=[]
        for feat in common:
            for grp in GROUPS:
                c_vals=child_df[child_df['Group']==grp][feat].dropna()
                w_vals=walk_child_df[walk_child_df['Group']==grp][feat].dropna()
                if len(c_vals)<3 or len(w_vals)<3: continue
                stat,p=stats.mannwhitneyu(c_vals,w_vals,alternative='two-sided')
                d=cohen_d(c_vals.values,w_vals.values)
                rows.append({'feature':feat,'Group':grp,
                             'cruising_median':float(c_vals.median()),
                             'walking_median':float(w_vals.median()),
                             'mw_p':float(p),'cohens_d_cruise_vs_walk':d,
                             'n_cruising':len(c_vals),'n_walking':len(w_vals)})
        if rows:
            cruise_walk_comparison=fdr_annotate(pd.DataFrame(rows),'mw_p')
            cruise_walk_comparison.to_csv(
                os.path.join(OUTPUT_DIR,'cruising_vs_walking_comparison.csv'),index=False)
            print(f"  Sig differences: {cruise_walk_comparison['sig_raw05'].sum()}")
else:
    print("  Walking CSV not found — skipping comparison.")
...
fig.suptitle('Cruising v3 — Sample Overview',fontweight='bold')
...
axes[2].set_title('(c) Cruising clips per child'); axes[2].set_xlabel('N clips'); axes[2].legend()
...
if len(lme_all)>0:
    ...
    res_plot['label']=res_plot['feature'].map(CRUISING_FEAT_SHORT).fillna(
        res_plot['feature'].str.replace('_',' '))
    ...
    ax.set_title(f"Effect Sizes — {method_label} + random slope + Group×Age\n★=FDR sig  Bars=95% bootstrap CI",
                 fontweight='bold')
...
if len(consensus_all)>0:
    ...
    ax.set_title('Consensus p-values — Cruising (top 20 features)',fontweight='bold')
    plt.tight_layout(); savefig(fig,'fig3_consensus_heatmap.png')

# Fig 4: Violin plots — top 12 by LME
...
if top12_feats:
    ...
    fig.suptitle('Top Cruising Features — ASD vs Non-ASD',fontweight='bold')
    axes=axes.flatten()
...
BOX_FEATS=[f for f in ['hip_x_amplitude','trunk_lateral_lean_amplitude',
                        'step_width_mean','lateral_cadence','wrist_x_lr_asym']
           if f in child_df.columns]
if BOX_FEATS:
    fig,axes=plt.subplots(1,len(BOX_FEATS),figsize=(4.5*len(BOX_FEATS),5))
    if len(BOX_FEATS)==1: axes=[axes]
    fig.suptitle('Child-Level Cruising Averages (each dot = one child)',fontweight='bold')
...
STRAT_PLOT=[f for f in ['hip_x_amplitude','trunk_lateral_lean_amplitude',
                         'lateral_cadence','step_width_mean','wrist_x_lr_asym']
...
if STRAT_PLOT:
    ...
    fig.suptitle('Cruising Features by Age Band',fontweight='bold')
...
if len(cons_df)>0:
    ...
    ax.barh(cons_df['feature'].map(CRUISING_FEAT_SHORT).fillna(cons_df['feature']),
...
if 'full' in bayes_main_results and len(bayes_main_results['full'])>0:
    ...
    bdf['label']=bdf['feature'].map(CRUISING_FEAT_SHORT).fillna(bdf['feature'])
...
TRAJ_FEATS=[f for f in ['hip_x_amplitude','trunk_lateral_lean_amplitude',
                         'lateral_cadence','step_width_mean','wrist_x_lr_asym',
                         'elbow_angle_L_mean','support_arm_reach_L_mean','knee_angle_L_range']
            if f in feat_df.columns]
if TRAJ_FEATS:
    ncols=4; nrows=int(np.ceil(len(TRAJ_FEATS)/ncols))
    fig,axes=plt.subplots(nrows,ncols,figsize=(6*ncols,4.5*nrows))
    fig.suptitle('Developmental Trajectories — Cruising',fontweight='bold')
    axes=axes.flatten()
...
if all_clf_results:
    ...
    fig.suptitle('Classification ROC — Child-Level LOSO — Cruising',fontweight='bold')
...
if len(feat_importance_df)>0:
    ...
    ax.barh(top20['feature'].map(CRUISING_FEAT_SHORT).fillna(top20['feature']),
            top20['importance'],color=ASD_COLOR,edgecolor='white',height=0.65,alpha=0.85)
    ax.set_xlabel('Mean decrease in impurity')
    ax.set_title('RF Feature Importances — Cruising (child level)',fontweight='bold')
...
if len(r_clip_mwu) and len(mw_all):
    ...
    ax.set_title('Pseudoreplication Diagnostic — Cruising\nPoints above y=x = clip MWU anti-conservative',fontweight='bold')
    ax.legend(fontsize=9)
    plt.tight_layout(); savefig(fig,'fig13_pseudoreplication_check.png')

# Fig 14: Cruising vs Walking comparison
print("Fig 14: Cruising vs Walking comparison...")
if walk_child_df is not None and len(cruise_walk_comparison)>0:
    common_feats=cruise_walk_comparison['feature'].unique()[:8].tolist()
    fig,axes=plt.subplots(2,4,figsize=(18,9))
    fig.suptitle('Cruising vs Walking — Shared Feature Comparison',fontweight='bold')
    axes_flat=axes.flatten()
    for i,feat in enumerate(common_feats):
        if i>=len(axes_flat): break
        ax=axes_flat[i]
        for j,grp in enumerate(GROUPS):
            c_vals=child_df[child_df['Group']==grp][feat].dropna().values if feat in child_df.columns else np.array([])
            w_vals=walk_child_df[walk_child_df['Group']==grp][feat].dropna().values if feat in walk_child_df.columns else np.array([])
            if len(c_vals)>0:
                ax.boxplot(c_vals,positions=[j*2],widths=0.4,patch_artist=True,
                           boxprops={'facecolor':COLORS_LIGHT[grp]},
                           medianprops={'color':COLORS[grp],'linewidth':2},showfliers=False)
            if len(w_vals)>0:
                ax.boxplot(w_vals,positions=[j*2+1],widths=0.4,patch_artist=True,
                           boxprops={'facecolor':'white','edgecolor':COLORS[grp],'linewidth':2,'hatch':'//'},
                           medianprops={'color':COLORS[grp],'linewidth':2},showfliers=False)
        ax.set_xticks([0.5,2.5]); ax.set_xticklabels(GROUPS,fontsize=9)
        ax.set_title(FEAT_LABEL(feat),fontsize=9,fontweight='bold')
        ax.legend(handles=[mpatches.Patch(color='gray',label='Cruising'),
                            mpatches.Patch(facecolor='white',edgecolor='gray',hatch='//',label='Walking')],
                  fontsize=7)
    for j in range(len(common_feats),len(axes_flat)): axes_flat[j].set_visible(False)
    plt.tight_layout(); savefig(fig,'fig14_cruising_vs_walking.png')
...
hr("CRUISING ANALYSIS v3 COMPLETE")