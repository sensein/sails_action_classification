...
def print_memory_debug(stage: str = "", verbose: bool = True) -> dict[str, float]:
...
def extract_frames_from_video(
    video_path: str, output_dir: str, target_fps: float | None = None
...
def inference_video_vps_save_results(
    ...
    pred_stability_scores: torch.Tensor | None,
    pred_masks: torch.Tensor,
    out_size: tuple[int, int],
    overlap_threshold: float = 0.5,
    mask_binary_threshold: float = 0.5,
    object_mask_threshold: float = 0.03,
    ...
    """
    print("[Post-processing] Starting inference_video_vps_save_results")
    ...
    topk_score = scores.topk(k=min(len(scores), test_topk_per_image))[0][-1].item()
    threshold = float(max(object_mask_threshold, topk_score))
    ...
    import atexit
    panoptic_seg: np.memmap | torch.Tensor  # assigned in either branch below
    use_memmap = False
    memmap_path = None

    if estimated_size > 10.0:
        print("[Post-processing] Using memory-mapped file (size > 10 GB)")
    ...
    segments_infos: list[dict[str, object]] = []
    ...
    print("[Post-processing] Pass 1 complete.")
    ...
    # ---- Build keep list -----------------------------------------------------
    keep_entries: list[tuple[int, int, int]] = []
...
def create_video_from_masks(
    pan_format: np.ndarray, output_video_path: str, fps: float = 30
) -> None:
    """
...
def process(
    ...
    categories_dict: dict[int, dict[str, object]],
    output_dir: str,
    video_dir: str,
    fps: float = 30,
) -> dict[str, object]:
    ...
    img_shape      = cast(tuple[int, int], outputs["image_size"])
    pan_seg_result = cast("np.ndarray | torch.Tensor", outputs["pred_masks"])
    segments_infos = cast(list[dict[str, object]], outputs["segments_infos"])
    segments_infos_ = []
    ...
    for seg_info in segments_infos:
        seg_id = int(cast(int, seg_info["id"]))
        sem    = int(cast(int, seg_info["category_id"]))

        # pan_seg_result may be a torch tensor or numpy memmap
        pan_seg_np = pan_seg_result.numpy() if isinstance(pan_seg_result, torch.Tensor) else np.asarray(pan_seg_result)
        mask = pan_seg_np == seg_id
        color = color_generator.get_color(sem)
        pan_format[mask] = color

        dt_base = {"category_id": sem - 1, "iscrowd": 0, "id": int(rgb2id(color))}
        dts: list[dict[str, object] | None] = []
    ...
    print("  Creating segmentation video without saving individual frames...")
...
def process_single_video(
    ...
    try:
        ...
        if os.path.exists(cache_path):
            ...
            init_pil   = Image.open(os.path.join(temp_dir, frame_names[0]))
            init_frame = np.array(init_pil.convert("RGB"))
        ...
        if "_memmap_path" in result:
            ...
            try:
                os.unlink(str(result["_memmap_path"]))
...
def main() -> None:
    ...
    torch.autocast(device_type="cuda", dtype=torch.bfloat16).__enter__()  # type: ignore
    ...
    # Load model
    from sam2.build_sam import build_sam2_video_query_iou_predictor  # lazy import
    ...
    for i, video_info in enumerate(video_chunks, 1):
        ...
        try:
            ...
            result: dict[str, object] = process_single_video(
    ...
    if all_results:
        total_frames = sum(cast(int, r["frames_processed"]) for r in all_results)
