...
def annotate_video(file_path, coordinates):
    ...
    # Load raw video
    from skvideo.io import FFmpegWriter, ffprobe, vreader
    videogen = vreader(file_path)
    video_metadata = ffprobe(file_path)['video']
...
input_folder = 'input'
output_folder = 'outputs/EfficientPose'
os.makedirs(output_folder, exist_ok=True)

default_model = 'II_Lite'
default_framework = 'TFLite'
default_visualize = True
default_store = True
#The code not work for mkv files. also by default the output is stored in the input folder
mkv_files = [f for f in os.listdir(input_folder) if f.endswith('.mp4')]
print(f"Found {len(mkv_files)} MKV files to process.\n")

for filename in mkv_files:
    input_path = os.path.join(input_folder, filename)
    output_path = os.path.join(output_folder, filename.replace('.mkv', '.mp4'))

    print(f'Processing "{filename}" with model "{default_model}", framework "{default_framework}", storing results: {default_store}, visualization: {default_visualize}')

    main(
        file_path=output_path,
        model_name=default_model, #EfficientPose model to utilize (RT, I, II, III, IV, RT_Lite, I_Lite or II_Lite)
        framework_name=default_framework, #framework to use (Keras, TensorFlow, TensorFlow Lite or PyTorch)
        visualize=True,
        store=True,
    )
