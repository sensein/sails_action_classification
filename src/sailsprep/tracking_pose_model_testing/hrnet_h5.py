...
# ============================================================
#  Argument parsing  (supports SLURM job arrays)
# ============================================================
...
# --- ViTPose-style online kp jump filter ---
KP_JUMP_THRESH   = 0.25
KP_MEMORY_FRAMES = 10

# --- ViTPose-style post-hoc compound suspicion filter ---
...
det_checkpoint = "/orcd/data/satra/002/models/mmdet/dino-5scale_swin-l_8xb2-36e_coco-5486e051.pth"
pose_config    = ("/home/aparnabg/orcd/scratch/mmpose/configs/wholebody_2d_keypoint/"
...
# ============================================================
#  HRNet 133-kp anatomical validation  (from HRNet script)
# ============================================================

def validate_pose_predictions_133(pose_results, bboxes):
    """
    ...
    validated = []
    for result, bbox in zip(pose_results, bboxes):
        ...
        shoulders_y = float(np.mean([s[1] for s in sh_valid])) if sh_valid \
                      else nose_y + bbox_h * 0.15

        # hip y
        hip_valid = [kps[i] for i in [L_HIP_IDX, R_HIP_IDX] if scores[i] > 0.3]
        hips_y = float(np.mean([h[1] for h in hip_valid])) if hip_valid \
                 else shoulders_y + bbox_h * 0.35

        # ---- 1. Leg / ankle checks (same as original HRNet script) ----
        ...
        knee_thresh = float(np.mean(knee_y_vals)) if knee_y_vals \
...
# ============================================================
#  H5 bbox loading  (identical to ViTPose script)
# ============================================================
...
# ============================================================
#  Bbox cleanup  (identical to ViTPose script)
# ============================================================

def _clean_one_pass(arr, suspect, window, edge_thresh, center_thresh,
                    neighbor_agree, ar_thresh):
    N = len(arr)
    cleaned = arr.copy()
    ...
    for i in range(N):
        ...
        if c_shift > center_thresh:
            ...
            current[0] = cx_med - w/2; current[1] = cy_med - h/2
            current[2] = cx_med + w/2; current[3] = cy_med + h/2
            n_centers += 1
            touched = True
        ...
        if current[2] <= current[0]: current[2] = current[0] + 1
        if current[3] <= current[1]: current[3] = current[1] + 1
...
def clean_bbox_map(bbox_map, window=7, edge_thresh=0.25, center_thresh=0.20,
                   neighbor_agree=0.25, ar_thresh=BBOX_AR_THRESH, n_passes=2):
    if not bbox_map:
    ...
    for _ in range(n_passes):
        ...
        tot_edges += ne; tot_centers += nc; tot_ar += na; tot_frames += nf
        if nf == 0:
            break
...
# ============================================================
#  Keypoint feature helpers  (identical to ViTPose script)
# ============================================================

def _kp_features(kmap):
    """kmap: {kp_name: (x, y, score)}"""
    if not kmap:
        return None
...
# ============================================================
#  Post-hoc compound suspicion filter  (identical to ViTPose)
# ============================================================

def _compound_score_one_pass(pose_store, bbox_map, frames, suspect,
    ...
    for i, f in enumerate(frames):
        ...
        for kp in kp_names:
            ...
            if len(b_v) >= 2 and len(a_v) >= 2:
                if np.linalg.norm(np.median(pos[b_v], 0) - np.median(pos[a_v], 0)) \
                        > diag * neighbor_agree:
                    continue
        ...
        if not np.isnan(centroid_arr[i, 0]):
            ...
            if len(nb_c) >= 3:
                ...
                stable = not (len(bc) >= 2 and len(ac) >= 2 and
                              np.linalg.norm(np.median(centroid_arr[bc], 0) -
                                             np.median(centroid_arr[ac], 0))
                              > diag * neighbor_agree)
                if stable:
        ...
        if not np.isnan(spread_ar_arr[i]):
            ...
            if len(nb_ar) >= 3:
                ...
                stable = not (len(bar) >= 2 and len(aar) >= 2 and
                              abs(np.median(spread_ar_arr[bar]) -
                                  np.median(spread_ar_arr[aar]))
                              / max(np.median(spread_ar_arr[nb_ar]), 1e-6)
                              > neighbor_agree)
                if stable:
                    med_ar  = np.median(spread_ar_arr[nb_ar])
                    ar_dev  = abs(spread_ar_arr[i] - med_ar) / max(med_ar, 1e-6)
                    score_C = min(ar_dev / 0.40, 1.0)
        ...
        if not np.isnan(bbox_ar_arr[i]):
            ...
            if len(nb_bar) >= 3:
                ...
                stable = not (len(bbar) >= 2 and len(abar) >= 2 and
                              abs(np.median(bbox_ar_arr[bbar]) -
                                  np.median(bbox_ar_arr[abar]))
                              / max(np.median(bbox_ar_arr[nb_bar]), 1e-6)
                              > neighbor_agree)
                if stable:
...
def post_filter_keypoints(pose_store, bbox_map,
                          window=POST_WINDOW,
                          jump_thresh=POST_JUMP_THRESH,
                          neighbor_agree=POST_NEIGHBOR_AGREE,
                          n_passes=POST_N_PASSES,
                          score_thresh=SUSPECT_SCORE_THRESHOLD,
                          kp_w=KP_SCORE_W, kpc_w=KPC_SCORE_W,
                          kpar_w=KPAR_SCORE_W, bboxar_w=BBOX_AR_SCORE_W):
    if not pose_store:
    ...
    cleaned = {f: ({} if f in flagged else pose_store[f]) for f in frames}
    return cleaned, flagged, int(suspect.sum())


# ============================================================
#  Build video list and slice for this array job
# ============================================================
...
for vid_idx, (video_path, h5_path) in enumerate(videos_to_run):
    ...
    bbox_map, n_edges, n_centers, n_ar, n_frames_fixed, per_pass = clean_bbox_map(
        raw_bbox_map,
    ...
    # step maps video frames → H5 annotation frames (e.g. 30fps video / 15fps H5 = step 2)
    ...
    # Inference pass
    # ----------------------------------------------------------
    pose_store   = {}   # ann_frame_idx -> {kp_name: (x, y, score)}
    last_kp      = {}   # online temporal memory
    pose_count   = 0
    error_count  = 0
    no_bbox_count = 0
    kp_dropped   = 0
    ...
    with tqdm(total=total, desc="  inference") as pbar:
        ...
        while cap.isOpened():
            ...
            # ann_f is the H5 index (15fps space)
            ann_f = vidx // step

            if ann_f not in bbox_map:
                no_bbox_count += 1
                vidx += 1; pbar.update(1); continue
            ...
            try:
                ...
                # --- detection (use H5 bbox directly, no re-detect needed) ---
                # We pass the H5 bbox directly as the person bbox to HRNet
                ...
                # --- HRNet anatomical validation ---
                pose_results = validate_pose_predictions_133(pose_results, bbox_arr)

                if not pose_results:
                    vidx += 1; pbar.update(1); continue

                # Use highest-confidence person (usually 1 person per bbox)
                result = pose_results[0]
                kps    = result.pred_instances.keypoints[0]    # (133, 2)
                scores = result.pred_instances.keypoint_scores[0]  # (133,)

                # Build kp_name -> (x, y, score) dict for this frame
                # kp names: use index strings to keep it generic & consistent
                # (HRNet wholebody doesn't have a built-in id2label like ViTPose,
                #  so we use "kp_000" … "kp_132" as stable keys)
                frame_kps = {}
                for ki in range(133):
                    s = float(scores[ki])
                    ...
                    # --- online temporal jump filter (same as ViTPose) ---
                    accepted = True
                ...
                # age up / expire memory for unseen keypoints
    ...
    # ----------------------------------------------------------
    # Post-hoc compound suspicion filter  (ViTPose-style)
    # ----------------------------------------------------------
    print("  Post-hoc compound suspicion filtering...")
    pose_store, flagged_frames, n_post_flagged = post_filter_keypoints(
        pose_store, bbox_map)
    print(f"  Post-filter removed {n_post_flagged} frames total")

    # ----------------------------------------------------------
    # Save JSON
    # Format mirrors ViTPose output exactly, with kp_000..kp_132 as names.
    # Keypoint index mapping:
    #   kp_000..kp_016 : COCO body (17 kp)
    #   kp_017..kp_022 : Foot      (6 kp)
    #   kp_023..kp_090 : Face      (68 kp)
    #   kp_091..kp_111 : Left hand (21 kp)
    #   kp_112..kp_132 : Right hand(21 kp)
    # Only frames with >=1 keypoint after all filtering are written.
    # ----------------------------------------------------------
    json_frames = {}
