"""Aggregate Ovis2 results across multiple seeds and compute metric spread.

Directory structure expected (created by submit_ovis_clip.sh):
    <results-dir>/
        seed_42/
...
Usage:
    python aggregate_ovis_results_multiseed.py --task loco --results-dir /path/to/output
    python aggregate_ovis_results_multiseed.py --task rmm  --results-dir /path/to/output
"""
...
def _merge_chunks(chunk_dir: str, class_names: list[str]) -> pd.DataFrame | None:
    """Merge clip_predictions_*.csv files in chunk_dir; return merged df or None."""
...
def _compute_metrics_for_df(
    ...
    """Compute all metrics for a merged predictions dataframe."""
...
def _compute_spread(seed_metrics: dict[str, dict]) -> dict:
    """Given {seed_label -> metrics_dict}, compute mean/std/min/max/CI per metric."""
    spread: dict = {}
    for metric in SPREAD_METRICS:
        ...
        # 95% CI via t-distribution (only meaningful with ≥2 seeds)
        if len(arr) >= 2:
            se = stats.sem(arr)
...
def _print_spread_table(spread: dict, task: str) -> None:
    """Pretty-print the spread table to console."""
    print(f"\n{'=' * 70}")
    print(f"METRIC SPREAD ACROSS SEEDS — {task.upper()}")
    print(f"{'=' * 70}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Aggregate Ovis2 results across seeds and compute spread.",
    )
    ...
    # Save a flat summary CSV (one row per seed, one column per metric)
    rows = []
