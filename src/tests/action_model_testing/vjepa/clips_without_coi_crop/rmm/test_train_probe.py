"""
Tests for sailsprep/action_model_testing/vjepa/clips_without_coi_crop/rmm/train_probe.py
...
from sailsprep.action_model_testing.vjepa.clips_without_coi_crop.rmm.train_probe import (
    EMBED_DIM,
    AttentiveProbe,
    FeatureDataset,
    run_inference,
...
class TestAttentiveProbe:
    def test_forward_shape(self):
        ...
        assert probe(x).shape == (3, 4)

    def test_batch_size_1(self):
        probe = AttentiveProbe(embed_dim=32, num_classes=2, num_heads=4)
        assert probe(torch.randn(1, 6, 32)).shape == (1, 2)
...
class TestTrainProbe:
    ...
    def test_runs_one_epoch_and_saves_log(self, tmp_path, monkeypatch):
        import sailsprep.action_model_testing.vjepa.clips_without_coi_crop.rmm.train_probe as mod
        monkeypatch.setattr(mod, "MAX_EPOCHS", 1)
        monkeypatch.setattr(mod, "PATIENCE", 1)

        probe = AttentiveProbe(embed_dim=EMBED_DIM, num_classes=4, num_heads=8)
        tr_dl = self._make_loader(num_classes=4)
        va_dl = self._make_loader(num_classes=4)
