"""
Tests for sailsprep/action_model_testing/vjepa/clips_without_coi_crop/rmm/train_probe_ablation.py
...
from sailsprep.action_model_testing.vjepa.clips_without_coi_crop.rmm.train_probe_ablation import (
    EMBED_DIM,
    HEAD_CHOICES,
    AttentiveProbe,
    FeatureDataset,
...
class TestClassificationHeads:
    def test_linear_probe_shape(self):
        probe = LinearProbe(embed_dim=32, num_classes=4)
        assert probe(torch.randn(2, 5, 32)).shape == (2, 4)
    ...
    def test_mlp_large_probe_shape(self):
        probe = MLPLargeProbe(embed_dim=32, num_classes=4)
        assert probe(torch.randn(2, 5, 32)).shape == (2, 4)
    ...
    def test_transformer_probe_shape(self):
        probe = TransformerProbe(embed_dim=32, num_classes=4, num_heads=4)
        assert probe(torch.randn(2, 5, 32)).shape == (2, 4)
...
class TestBuildProbe:
    def test_builds_each_registered_head(self):
        for head_name, cls in [
            ...
            probe = build_probe(head_name, embed_dim=32, num_classes=4)
...
class TestTrainProbe:
    def _make_loader(self, n=8, tokens=5, dim=EMBED_DIM, num_classes=4, batch_size=4):
    ...
    def test_runs_one_epoch_and_saves_log(self, tmp_path, monkeypatch):
        import sailsprep.action_model_testing.vjepa.clips_without_coi_crop.rmm.train_probe_ablation as mod
        monkeypatch.setattr(mod, "MAX_EPOCHS", 1)
        monkeypatch.setattr(mod, "PATIENCE", 1)

        probe = LinearProbe(embed_dim=EMBED_DIM, num_classes=4)
...
class TestRunInference:
    def test_writes_predictions_and_metrics(self, tmp_path):
        ...
        test_labels_enc = [i % 4 for i in range(n)]
        original_labels = [classes[i % 4] for i in range(n)]
