...
def _import_transforms():
    ...
    # .parent.parent = src/
    PROJECT_ROOT = Path(__file__).parents[4]  # adjust depth to reach sails_action_classification/
    src = PROJECT_ROOT / "src/sailsprep/action_model_testing/OpenTAD/end_to_end_custom.py"
...
def _import_run_locomotion():
    # ── 1. Same package-stub setup as _import_transforms ──────────────────
    pkg_name = "sailsprep.action_model_testing.OpenTAD"
    for part in ["sailsprep", "sailsprep.action_model_testing", pkg_name]:
        if part not in sys.modules:
            sys.modules[part] = types.ModuleType(part)
    ...
    # ── 2. Fix copy-paste bug: load run_locomotion.py, not end_to_end_custom.py
    PROJECT_ROOT = Path(__file__).parents[4]
    src = PROJECT_ROOT / "src/sailsprep/action_model_testing/OpenTAD/run_locomotion.py"
    assert src.exists(), f"Cannot find run_locomotion.py at: {src}"

    spec = importlib.util.spec_from_file_location(
        f"{pkg_name}.run_locomotion", src,
        submodule_search_locations=[]
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules[f"{pkg_name}.run_locomotion"] = mod   # register before exec
