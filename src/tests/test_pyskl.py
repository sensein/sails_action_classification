...
MODULE_ROOT = Path(__file__).parents[2] / "sailsprep" / "action_model_testing" / "pyskl"
...
class TestExtractLogits:
    ...
    def test_val_split_uses_val_dataset(self, module, stub_heavy_imports):
        ...
        stubs["pyskl.datasets"].build_dataloader.return_value = []
        stubs["pyskl.models"].build_model.return_value = MagicMock()
        stubs["mmcv.parallel"].MMDataParallel.return_value = MagicMock()
    ...
    def test_main_saves_pkl(self, module, stub_heavy_imports, tmp_path, monkeypatch):
        ...
        with patch("glob.glob", return_value=[fake_ckpt]), \
             patch.object(module, "extract_logits_from_model",
                          return_value=(np.zeros((10, 4)), np.zeros(10, dtype=int))), \
