...
# ---------------------------------------------------------------------------
# Stub out every heavy dependency BEFORE the module is loaded so that none
# of the sailsprep / cv2 / h5py import machinery runs.
# ---------------------------------------------------------------------------
...
class TestProcessAll:
    def test_filter_ids_restricts_processing(self, identifier: TargetIdentifier) -> None:
        ...
        with patch.object(identifier, "process_child", side_effect=fake_process_child):
            with patch.object(identifier, "save_global_summary"):
