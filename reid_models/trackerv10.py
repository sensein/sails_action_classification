...
#use no face deepface is not good 
...
class DetectionValidator:
    """
    Validates if detection is a real person, not photo/screen/TV.pro
...
class PostHocTrackFilter:
    """
    Post-processing filter for track refinement.
    
    Removes:
    1. Short-duration tracks (<2 seconds)
    2. Background people exceeding (adults + children) count from CSV
    3. Tracks failing quality metrics
    """
    
    def __init__(self, fps: float, min_duration_sec: float = 2.0):
        """
        Initialize post-hoc filter.
        
        Args:
            fps: Video frame rate
            min_duration_sec: Minimum track duration in seconds
        """
        self.fps = fps
        self.min_frames = int(fps * min_duration_sec)
    
    def filter_tracks(self, tracks: Dict[int, Dict], person_profiles: Dict[int, Dict],
                     max_main_persons: Optional[int] = None) -> Tuple[Dict[int, Dict], List[int]]:
        """
        Filter tracks using multiple criteria.
        
        Args:
            tracks: Dictionary of all tracks (active + lost)
            person_profiles: Dictionary of person profiles
            max_main_persons: Maximum main persons (adults + children from CSV)
        ...
        # ====================================================================
        # STEP 1: Remove short-duration tracks
        # ====================================================================
        for track_id in list(tracks.keys()):
            track = tracks[track_id]
            duration = track['last_seen'] - track['created_frame']
            
            if duration < self.min_frames:
                removed_ids.append(track_id)
                print(f"  Removing Track {track_id}: Too short ({duration} frames < {self.min_frames})")
        
        # ====================================================================
        # STEP 2: Remove background people if exceeding max count
        # Background = people beyond (adults + children) count
        # ====================================================================
        if max_main_persons is not None:
            remaining_tracks = {tid: t for tid, t in tracks.items() 
                              if tid not in removed_ids}
            
            if len(remaining_tracks) > max_main_persons:
                # Score tracks by importance metrics
                track_scores = {}
                ...
                for track_id, track in remaining_tracks.items():
                    ...
                    # Motion variance (higher = more active person)
                    centers = [(d['bbox'][0] + d['bbox'][2])/2 for d in track['detections']]
                    motion_variance = np.var(centers) if len(centers) > 1 else 0
                ...
                # Keep top N tracks (main persons)
                sorted_tracks = sorted(track_scores.items(), key=lambda x: x[1], reverse=True)
                tracks_to_keep = [tid for tid, score in sorted_tracks[:max_main_persons]]
                
                for track_id in remaining_tracks.keys():
                    if track_id not in tracks_to_keep:
                        removed_ids.append(track_id)
                        print(f"  Removing Track {track_id}: Background person "
                              f"(score={track_scores[track_id]:.1f})")
        
        # ====================================================================
        # STEP 3: Remove tracks with poor quality indicators
        # ====================================================================
        for track_id in list(tracks.keys()):
            ...
            # Check average keypoint confidence
            avg_keypoint_conf = []
            ...
            if avg_keypoint_conf:
                ...
                if overall_avg_conf < 0.3:  # Very low confidence
                    removed_ids.append(track_id)
                    print(f"  Removing Track {track_id}: Low keypoint confidence "
                          f"({overall_avg_conf:.2f})")
        
        # Remove tracks and profiles
    ...
    def reassign_track_ids(self, tracks: Dict[int, Dict], 
        ...
        """
        Reassign track IDs to be consecutive (1, 2, 3, ...).
        
        Args:
            tracks: Filtered tracks dictionary
            person_profiles: Person profiles dictionary
            
        Returns:
            Tuple of (old_to_new_id_map, new_tracks, new_profiles)
        """
        # Sort by creation frame for stable ID assignment
...
class FilteringConfig:
    ...
    stationary_threshold_pixels: float = 5.0
    enable_posthoc_filtering: bool = True  # NEW: Enable post-hoc filtering
    min_duration_seconds: float = 2.0  # NEW: Minimum track duration
...
class EnhancedTrackingModule:
    ...
    def apply_posthoc_filtering(self, fps: float, max_main_persons: Optional[int] = None):
        """
        ...
        """
        print("\n" + "="*70)
        print("POST-HOC TRACK FILTERING")
        print("="*70)
        
        # Combine all tracks
        all_tracks = {**self.active_tracks, **self.lost_tracks}
        
        print(f"Total tracks before filtering: {len(all_tracks)}")
        ...
        post_filter = PostHocTrackFilter(
            fps=fps,
            min_duration_sec=self.config.track_filtering.min_duration_seconds
        )
        
        # Filter tracks
        filtered_tracks, removed_ids = post_filter.filter_tracks(
            all_tracks, self.person_profiles, max_main_persons
        )
        
        print(f"\nRemoved {len(removed_ids)} tracks")
...
class EnhancedTrackingPipeline:
    ...
    def _signal_handler(self, signum, frame):
        ...
        gc.collect()
        sys.exit(0)
    # before post filter video dont know the other function is correct or not need to check
    ...
    def process_video(self, pose_json_path: str, video_path: str,
        ...
        Post-processing: Filter and reassign IDs
        Pass 2: Render video with final IDs
        """
        print(f"Processing: {pose_json_path}")
        print(f"Video: {video_path}")
        ...
        print(f"\n{'='*70}")
        print(f"PASS 1: TRACKING AND FEATURE EXTRACTION")
        print(f"{'='*70}")
            ...
            with tqdm(total=total_frames, desc="Pass 1: Tracking") as pbar:
                while frame_number < total_frames and not self._interrupted:
                    ...
                    try:
                        ...
                        # Store tracking data (original IDs)
                        ...
                        if self.tracking_module.frame_count % 50 == 0:
                            ...
                            print(f"\nFrame {self.tracking_module.frame_count}: "
                                  f"Active={len(self.tracking_module.active_tracks)}, "
                                  f"Candidates={len(self.tracking_module.candidate_tracks)}, "
                                  f"Lost={len(self.tracking_module.lost_tracks)}, "
                                  f"Total={len(self.tracking_module.person_profiles)}")
        ...
        # ====================================================================
        # POST-PROCESSING: Validate, filter, and reassign IDs
        # ====================================================================
        print(f"\n{'='*70}")
        print(f"POST-PROCESSING: VALIDATION AND FILTERING")
        print(f"{'='*70}")
        
        old_to_new_mapping = {}
        
        # Validate tracks (detect photos/screens)
        invalid_track_ids = []
        if self.tracking_module.detection_validator:
            ...
            if cap_validation.isOpened():
                # Sample validation on last frame
                ...
                if ret:
                    ...
                    if invalid_track_ids:
                        print(f"  Found {len(invalid_track_ids)} invalid tracks (photos/screens)")
                cap_validation.release()
        
        # Apply post-hoc filtering if enabled
        if self.config.track_filtering.enable_posthoc_filtering:
            max_main_persons = self.config.tracking.max_persons
            if max_main_persons and max_main_persons >= 999:
                max_main_persons = None  # Disable for large crowds
            
            # This will filter and reassign IDs
        ...
        if self.config.visualization.enable_visualization and output_video_path:
            print(f"\n{'='*70}")
            print(f"PASS 2: RENDERING VIDEO WITH FINAL IDs")
            print(f"{'='*70}")
                    ...
                    with tqdm(total=len(frame_tracking_data), desc="Pass 2: Rendering") as pbar:
                        for frame_data in frame_tracking_data:
                            ...
                            for det_idx, old_track_id in original_assignments.items():
                                ...
                                # Map to new ID (skip if filtered out)
        ...
        if self.config.visualization.enable_visualization and output_video_path:
            print(f"  Pass 2 (rendering): {(processing_time - pass1_time):.2f}s")
    ...
    def _draw_tracking_final(self, frame: np.ndarray, detections: List[Dict],
        ...
        Returns:
            np.ndarray: Frame with tracking visualization showing final IDs
        """
        vis_frame = frame.copy()
        ...
        for det_idx, track_id in person_assignments.items():
            if det_idx < len(detections):
                ...
                # Default color: Green for all final tracks
                color = (0, 255, 0)  # Green
                ...
                # Draw text
                cv2.putText(vis_frame, label_text, (x1 + 2, y1 - 5),
