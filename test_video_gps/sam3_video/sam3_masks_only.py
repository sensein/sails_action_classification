        ...
        with torch.inference_mode():
            ...
            with ctx:
                for response in predictor.handle_stream_request(
                    ...
                    # ── masks → numpy uint8 (N, sam_h, sam_w) ─────────────
                    if masks is not None and len(obj_ids) > 0:
                        ...
                        while masks_np.ndim > 3:
                            masks_np = masks_np[:, 0]
                        # upsample to SAM working resolution if needed
...
def run_transformers(model, processor, device,
    ...
    # Load all frames into RAM (RGB for transformers)
    print(f"    Loading {total} frames via OpenCV ...")
    ...
    try:
        with torch.inference_mode():
            ...
            with ctx:
                for model_out in model.propagate_in_video_iterator(
                    ...
                    max_frame_num_to_track=total,   # no limit
                ):
                    frame_idx = model_out.frame_idx
                    ...
                    # free boxes tensor immediately — not needed
                    del proc["boxes"]
        ...
        print(f"    ⚠ OOM at frame ~{oom_frame} — partial results saved so far")
