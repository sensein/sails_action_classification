...
def peek_total_frames(video_path):
...
class NativeBackend:
    ...
    def _process_session(self, video_path, prompt, frame_reader,
    ...
    def process_video(self, video_path, prompt, output_dir,
        ...
        frame_reader = LazyFrameReader(video_path, resize_shorter_side=resize_shorter_side)
...
def main():
    ...
    if args.array_total > 1:
        video_paths = [v for i, v in enumerate(video_paths)
                       if i % args.array_total == args.array_index]
        print(f"  Job array [{args.array_index + 1}/{args.array_total}]: "
              f"this job will process {len(video_paths)} of {total_in_csv} videos "
    ...
    for i, video_path in enumerate(video_paths):
        ...
        try:
            n_frames = peek_total_frames(video_path)
        except Exception:
            n_frames = 0
    ...
    global_summary = {
        ...
        "this_job_attempted":  len(video_paths),
