...
# Configuration
INPUT_DIR = Path("/orcd/data/satra/002/projects/SAILS/feature_processing/pipeline_outputs/face_body_perfect_one_child_1or0_adult/tracking")
OUTPUT_VIDEO_DIR = Path("/orcd/data/satra/002/projects/SAILS/feature_processing/pipeline_outputs/face_body_perfect_one_child_1or0_adult/child_classifications/videos")
OUTPUT_LOG_DIR = Path("/orcd/data/satra/002/projects/SAILS/feature_processing/pipeline_outputs/face_body_perfect_one_child_1or0_adult/child_classifications/logs")

class ChildIdentificationProcessor:
    def __init__(self, config: ChildIdentificationConfig):
        self.config = config
        self.setup_logging()

    def setup_logging(self):
        """Setup logging to both file and console"""
        log_file = OUTPUT_LOG_DIR / f"batch_processing_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def save_detailed_log(self, filename: str, child_result: ChildResult,
        ...
        log_path = OUTPUT_LOG_DIR / f"{filename}_analysis.json"
    ...
    def process_single_file(self, json_path: Path) -> bool:
        ...
        try:
            ...
            # Create output video
            output_video_path = OUTPUT_VIDEO_DIR / f"{filename}_child_identified.mp4"
    ...
    def process_batch(self, max_files: Optional[int] = None, test_mode: bool = False, max_workers: Optional[int] = None, aggressive_sampling: bool = False):
        ...
        json_files = list(INPUT_DIR.glob("*_tracking.json"))
...
def main():
    ...
    processor = ChildIdentificationProcessor(config)
