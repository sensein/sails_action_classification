...
class FeatureExtractionModule:
    ...
    def __init__(self, config: FeatureConfig):
        ...
        # Use the simplified ResNet extractor
        if config.enable_upper_body_features or config.enable_lower_body_features:
...
class TrackingModule:
    ...
    def _match_with_appearance(self, unmatched_detections: List[Tuple[int, Dict]]) -> Dict[int, Tuple[int, str]]:
        ...
        #TODO: should potentially switch to hungarian assignment here to increase accuracy of assignments
        for det_idx, detection in unmatched_detections:
            ...
            for track_id, track in self.active_tracks.items():
                if track_id in matches.values() or track['last_seen'] == self.frame_count:
                    continue
...
def create_batch_config(json_output_path: str) -> 'PipelineConfig':
    ...
    config.features.feature_update_interval = 5
    ...
    # detection
    config.processing.detection_confidence_threshold = 0.4  # Person detection confidence (0.0-1.0) - lower=more detections
    config.processing.nms_threshold = 0.5                   # Non-max suppression (0.0-1.0) - lower=more overlap removal
    config.processing.min_height = 50                        # Minimum detection box height in pixels
    config.processing.min_width = 50                         # Minimum detection box width in pixels
    # motion tracking
    ...
    config.processing.combined_reid_threshold = 0.3          # Combined ReID threshold (0.3-0.9) - lower=more matches
    # save
    ...
    config.visualization.keypoint_threshold = 0.3
...
def main():
    ...
    VID_LOCAL_PATH = "/A.M._Home_Videos_AMES_B4Q3G8H2N3/34-38 month videos/IMG_1845.mp4"
    TARGET_VIDEO_PATH = "/orcd/data/satra/001/users/brukew/motion_tracking_output/reid_test_det_06_new_params.mp4"
